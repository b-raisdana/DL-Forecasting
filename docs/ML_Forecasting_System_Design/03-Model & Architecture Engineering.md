...
- **residual-CNN time-series-classification baselines** — three separately-implemented network topologies from the UCR/UEA time-series-classification benchmark literature, distinct lineage from the NLP-derived TCN/ModernTCN line above; different classes in any TSC library (e.g. `aeon`/`tsai`), not one function with a parameter choice — see [prioritization framework](05-Prioritization Framework.md#local-feature-extraction) for why each is scored independently rather than as one candidate:
...
Scored in [prioritization framework](05-Prioritization Framework.md#local-feature-extraction--placement); reuses whichever block (`plain_cnn`/`tcn_dilated`/etc.) wins the placement-agnostic technique comparison above, so this axis is "where," not "which."
...
- LSTM recurrence — sanity-check floor: mature, low-risk, well-understood mechanism, sequential context without attention's O(n²) cost.
  - **GRU** — `tf.keras.layers.GRU`, a separate layer class from `LSTM` (merged forget/input gate, no separate cell state, ~25% fewer parameters), not a parameter of the same layer — see [prioritization framework](05-Prioritization Framework.md#tool-identity-test-when-a-xy-grouping-stays-one-row) for why this and xLSTM/ConvLSTM below get the same "alt within the floor role" treatment despite one being a different class and the others being config choices. Tested as an alt within this same floor role, not a new pipeline stage.
...
- state-space — linear-time long-context alternative to attention. Two separately-implemented mechanisms, not one function with a parameter (see [prioritization framework](05-Prioritization Framework.md#sequential-encoding)):
...
- **TFT (Temporal Fusion Transformer)** and **Perceiver** — named in "combination strategy" below; not yet profiled with S1/S2/S3 hyperparams. Two separate architectures, scored independently: Perceiver's latent-bottleneck mechanism is already independently scored highest in this doc's [multi-timeframe fusion](05-Prioritization Framework.md#multi-timeframe-fusion) table for directly targeting the flagged O(n²)/VRAM ceiling, while TFT is the more field-dominant but less bottleneck-targeted of the two — see [prioritization framework](05-Prioritization Framework.md#current-stage-1-candidate-set) for the scoring.
...
This doc is where every candidate/combined-model design has to actually fit, so the budget lives next to what it constrains rather than in the higher-level planning doc. For the operational side — actually measuring GPU/VRAM/RAM/CPU/disk usage during a real training run, KPI thresholds, and bottleneck diagnosis — see [06-ML-Ops.md § training-run resource monitoring](<06-ML-Ops.md#training-run-resource-monitoring>).

- RTX 4060 Laptop GPU, 8GB VRAM (8188 MiB per `nvidia-smi`), 64GB RAM (host), 2 SSD/HDD. **Training itself runs inside WSL2**, which by default only exposed ~31GB of that 64GB to the process (no `.wslconfig` override existed) — found and fixed 2026-08-16 via `C:\Users\<user>\.wslconfig`'s `memory=48GB`, WSL2 now reports ~47GB; see [06-ML-Ops.md](<06-ML-Ops.md#training-run-resource-monitoring>) for the full finding. Still short of the raw 64GB host figure (Windows host + other WSL distros share the same VM-wide pool), so the [vram/ram budget split](#vramram-budget-split) below is sized off ~48GB, not 64GB.
...
- **RAM (64GB host / ~48GB configured for WSL2 via `.wslconfig` — see [hardware constraints](#hardware-constraints) above)**: percentages below are against the ~48GB WSL2 ceiling (was ~31GB before that fix), not the host's raw 64GB: ~10% OS/Python/dataloader-worker overhead, ~65% in-memory feature/candle cache (~31GB, vs. ~42GB a naive 64GB-based calculation would assume — still re-check "full or near-full multi-tf dataset stays resident" against this number as the dataset grows), ~15% pinned staging buffers for CPU→GPU transfer, ~10% headroom.
...
- **CNN-LSTM(-attention)** — see [current Stage-1 candidate set](#current-stage-1-candidate-set). A useful worked illustration of the skeleton itself: its plain and attention variants differ by exactly one `stage_config` value (`attention: 0` vs `attention: "self_attn"`), which is the zeroing mechanism working as designed, not a coincidence.
  ...
  - S1/S2/S3 profiles not yet assigned — backfill via `profile_trial_cost()` same as the others before including it in the Optuna study, not hand-picked; the `conv_layers` list above is a starting-point anchor, not a profiled S1/S2/S3 spread yet.
    Alt:
  - **residual-CNN time-series-classification baselines** as the `local_extraction` block instead of the plain stacked-conv above — same slot, three separate implementations, not one: **ResNet** (residual skip connections), **FCN** (global-pooled fully-conv stack, no residual), **InceptionTime** (multi-kernel-size Inception modules); see [local feature extraction](#local-feature-extraction) and the scoring in [prioritization framework](05-Prioritization Framework.md#local-feature-extraction). None yet reduced to a `stage_config`-ready block here — flagged as a follow-up, not designed in this pass.
...
- **state-space: Mamba** — cheap long-seq alt to attention. (**S4** is a lower-priority alt within this same `sequential: "ssm"` slot — swap `MambaBlock` below for an `S4Block`-equivalent; a separate implementation, not a parameter of `MambaBlock`, per [sequential encoding](#sequential-encoding) and the scoring in [prioritization framework](05-Prioritization Framework.md#sequential-encoding).)
...
- **LSTM** — sanity-check floor (S3 focus = bidirectional context, not receptive field — RNN context is sequential, not attention-based)
  ...
  - dropout: 0.1 (all profiles)
  - **GRU** (`cell_type="gru"` above) — Tier-2 alt tested within this same floor role, not a separate candidate slot; see [sequential encoding](#sequential-encoding) and the scoring in [prioritization framework](05-Prioritization Framework.md#sequential-encoding).
    Alt:
...
- A candidate with no row here isn't disqualified, but its `domain_fit` score should reflect that it's a generic capability bump rather than a targeted fit (see [scoring factors](05-Prioritization Framework.md#scoring-factors) factor 5).
...
4. **optimization difficulty** — loss curve unstable, non-monotonic, or NaN/Inf despite reasonable capacity/bias. → gradient flow (residual connections around new blocks, per [design layers to pass](#design-layers-to-pass) step 5), LR/warmup, mixed-precision numerics; ties to [04 § Training Engineering](<04-Experimentation, Evaluation & Optimization.md#training-engineering>) training stability.
...
6. **bad labels** — cross-check against label-quality/noise measurement ([99-Weakness Analysis.md § A3](<99-Weakness Analysis.md>)); a label-noise ceiling looks identical to "no architecture can fit this" from inside the architecture comparison alone.

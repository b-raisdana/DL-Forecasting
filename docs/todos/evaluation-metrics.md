...
Closing the gap between [error-rating-and-evaluation.md](../error-rating-and-evaluation.md) (the metrics
...
1. **Write a `backtest-module-design.md`** before any code: fill-simulation assumptions (matching
   [training-data.md § targeting bid price](../training-data.md#targeting-bid-price)'s limit-order
   ...
   [validation & train/test splitting](../model-architecture-planning.md#validation--traintest-splitting)
...
3. **Implement `expectancy/trade`** (primary KPI) from simulated trades — average R-multiple/% gained
   ...
   [error-rating-and-evaluation.md § backtested trading KPIs](../error-rating-and-evaluation.md#backtested-trading-kpis-final-selection).
...
7. **Wire per-head statistical metrics** (dev diagnostics, not selection criteria) per
   [error-rating-and-evaluation.md § per-head statistical metrics](../error-rating-and-evaluation.md#per-head-statistical-metrics-dev-diagnostics):
...
9. **Build the statistical-validity harness**: ≥3 seeds/config (5 preferred), paired stat test across
   ...
   [error-rating-and-evaluation.md § statistical validity of comparisons](../error-rating-and-evaluation.md#statistical-validity-of-comparisons).
...
10. **Wire the model-selection pipeline end to end**: search-time per-head losses → finalist re-run
    ...
    exactly once, per
    [error-rating-and-evaluation.md § model-selection pipeline](../error-rating-and-evaluation.md#model-selection-pipeline).
...
11. **(decision) Confidence/calibration metric technique.** No input feature carries confidence
    ...
    [model-architecture-planning.md § uncertainty-native GBM variants](../model-architecture-planning.md#uncertainty-native-gbm-variants--confidence-metric-gap)
    ...
    [error-rating-and-evaluation.md § confidence & calibration metrics](../error-rating-and-evaluation.md#confidence--calibration-metrics).
...
- **No backtest module exists anywhere in the codebase.** `vectorbt` is listed in
  ...
  [error-rating-and-evaluation.md](../error-rating-and-evaluation.md) is therefore aspirational, not

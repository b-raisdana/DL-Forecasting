...
Replaced with `scripts/git-hooks/incremental-precommit/` (full design in that folder's README): two independent layers, both recomputed fresh every commit. The blocking gate is a per-touched-file diff - each touched file's own violation count (`mypy`/`ruff`/`xenon`) or line count (`loc`) is compared before this commit (via a throwaway `git worktree` at `HEAD`) vs. after, and only files that got worse block; zero tolerance for `mypy`/`ruff`/`xenon`, a 5-line slack for `loc` on files already over 500 lines, a hard 500-line cap for brand-new files. This can't be masked by unrelated improvement elsewhere (fixing ten violations of one kind can't hide a new violation of another in the same commit) and has no "first time this's been seen" bootstrap loophole. A project-wide count per **key** in `baseline.json` (`mypy`/`ruff` split one key per rule/error code, `xenon`/`loc` single counts) is kept only as a trend metric feeding the mutation-safety reminder below - it never blocks a commit by itself, and resyncs to the fresh counts after every successful commit (self-pruning: a key with nothing left drops out).
...
- **Not the disk-cache migration target**: `infrastructure.disk_cache`'s generic `(data_frame_type, date_range_str)` engine backs ~20+ callers (`domain/price_action/*`, `domain/schemas/common/ExtendedDf.py`, ...) in a single-process offline pipeline with no other concurrent consumer — a client-server store adds daemon/port management this repo's architecture (`infrastructure.md` § SOA) doesn't call for. DuckDB gets the same range-query win over Parquet with zero server — see below, adopted instead.

### DuckDB

Embedded, zero-server OLAP query engine — the option ClickHouse's own entry above was parked in favor of. Used as a **batched read layer** over `data/dataset_db/`'s existing Parquet/ZSTD cache, not a storage format or architecture change: writes still go through `infrastructure.disk_cache.write_data_file()` unchanged, and DuckDB only reads what's already on disk.

- **Where**: `infrastructure/duckdb_reader.py` (`read_parquet_files()`) — takes an explicit list of already-on-disk Parquet files for one `data_frame_type` plus a `[start, end]` range, runs one `SELECT * FROM read_parquet($files) WHERE date >= ? AND date <= ?` query (real row-group pruning on the `date` column every cache file carries), and indexes the result identically to `disk_cache.read_by_date()`/`read_with_timeframe()`.
- **Wired into**: `infrastructure/disk_cache_windowed.py`'s `read_file_windowed()` — the windows of a requested range that are already fully cached (the common case for repeated backtesting/training reads over historical data) are batched through one DuckDB query instead of `read_file_windowed()`'s previous one-`pd.read_parquet()`-call-per-window loop; windows still needing generation, legacy-file seeding, or a future span go through `read_file()` individually, unchanged. Falls back to the per-window path on any DuckDB read/validation failure (rare — every on-disk file was already validated at write time), so `read_file()`'s self-healing-via-regeneration guarantee isn't lost for a bad file.
- **Why `data_frame_type`-first partitioning helps**: `data/dataset_db/<data_frame_type>/<Market>/<Symbol>/<Exchange>/` (see `data/dataset_db/README.md`) puts the type outermost, so a query can glob one type across every symbol/market/exchange in a single call — the layout this batching (and any future cross-symbol query) relies on.
- **Dependency**: `duckdb` (`requirements.txt`), embedded — no `docker-compose.yml` service, no port, no daemon.
- **Tradeoff**: batched windows skip populating `infrastructure.disk_cache`'s per-window `_read_file_cache` LRU memo (32 entries) — acceptable since DuckDB reads straight from row-group-pruned Parquet, comparable cost to a memo hit.
- **Out of scope for now**: `infrastructure/disk_cache_gaps.py`'s gap/overlap detection still does plain directory-listing + filename regex matching, not a DuckDB catalog query — it's not a performance bottleneck today, so folding it in is a documented future follow-up, not done as part of this change.

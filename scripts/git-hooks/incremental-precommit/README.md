...
([config.json](config.json), default 3), `ratchet_check.py` rewrites `baseline.json` to the new,
...
function's _current actual_ output with a characterization test before changing it. When a commit
...
Considered (e.g. `mypy-baseline`, ignore-comment sprinkling) and rejected: those mark _specific
lines_ as pre-existing debt, which is more precise but requires generating and maintaining a
...
app/**init**.py exists) and mypy aborted after 1 file ("found twice").

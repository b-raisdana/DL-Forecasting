...
def batch_generator_zip_pkl(
    ...
    while True:
        ...
        log_d("All listed files read at-least once.")


def generate_batch_zip_pkl(
    batch_size: int, mt_ohlcv: pt.DataFrame[MultiTimeframe], x_shape: dict[str, tuple[int, int]], save=False
):
    Xs, ys, X_dfs, y_dfs, y_timeframe, y_debug_dfs = train_data_of_mt_n_profit(
        structure_tf="4h",
        mt_ohlcv=mt_ohlcv,
        x_shape=x_shape,
    ...
    if save:
        ...
        save_validators_zip_pkl(
            X_dfs, y_dfs, y_timeframe, y_debug_dfs, folder_name, app_config.under_process_symbol, timestamp
        )
...
def save_validators_zip_pkl(
    X_dfs: dict[str, list[pd.DataFrame]],
...
def load_validators_zip_pkl(
    folder_path: str, picked_file: str
...
def training_dataset_main():
    ...
    app_config.processing_date_range = date_range_to_string(
        start=pd.to_datetime("03-01-24"), end=pd.to_datetime("09-01-24")
    )

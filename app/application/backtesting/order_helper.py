...
def dict_of_order(order: bt.Order):
    ...
    for key, value in order.__dict__.items():
        if not key.startswith("_") and type(value) != bt.OrderData:
            if key == "dt":
    ...
    hidden_keys = [
        ...
        in [
            ...
            "created_remsize",
            "created_trailamount",
            "created_trailpercent",
            "created_value",
            "executed_p1",
            "executed_p2",
            "executed_pricelimit",
            "executed_trailamount",
            "executed_trailpercent",
            "info",
            "executed_margin",
            "comminfo",
            "executed_exbits",
...
def add_order_info(
    order: bt.Order, signal: pd.Series, signal_index: pd.MultiIndex, order_type: BracketOrderType, order_group_id
):

...
class MultiHeadAttentionLayer(tf.keras.layers.Layer):  # type: ignore[misc]
    def __init__(self, num_heads: int, key_dim: int, **kwargs: object) -> None:
        super().__init__(**kwargs)
        self.num_heads = num_heads
        self.key_dim = key_dim
    ...
    def call(self, inputs: tf.Tensor) -> tf.Tensor:
        ...
        # split into heads
        def split_heads(x: tf.Tensor) -> tf.Tensor:
            x = tf.reshape(x, (batch_size, -1, self.num_heads, self.key_dim))

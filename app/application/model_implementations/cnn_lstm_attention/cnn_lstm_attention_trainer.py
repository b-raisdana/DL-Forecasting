...
from application.dataset_generation.feeders.ram_batch import build_ram_dataset
from application.model_implementations.cnn_lstm_attention.cnn_lstm_attention_model import (
    ...
    if model is None:
        if not rebuild_model and os.path.exists(model_path_keras):
            ...
            model = build_cnn_lstm_attention_model(
                ...
                cnn_count=cnn_count,
                kernel_step=cnn_kernel_growing_steps,
...
if __name__ == "__main__":
    ...
    try:
        while True:  # infinite loop
            ...
            p = mp.Process(
                target=run_cnn_lstm_attention_trainer, args=(training_round_counter,)
            )  # 1️⃣ start a brand‑new process
            p.start()

...
"""
Orchestrates infrastructure.datastore_engine.parquet_housekeeping's two file-level jobs — legacy
...
batch run over dataset_db, driven by presentation.datastore_engine.parquet_housekeeping_cli. One bad file
...
def run_datastore_conversion(root: Path | None = None, dry_run: bool = False) -> ConversionSummary:
    ...
    infrastructure.datastore_engine.parquet_housekeeping module docstring). `dry_run=True` reports what
    ...
    log_i(
        f"parquet_housekeeping: {verb} {len(converted)} legacy file(s) "

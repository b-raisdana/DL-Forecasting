...
pattern shared by windowing/cleanup/gap-discovery; the redundant-file cleanup pass; and the
NaN-column bookkeeping behind write_data_file()'s write-time guard.
"""
...
def _data_frame_type_dir(data_frame_type: str, file_path: str) -> str:
    """
    ...
    disk_cache.py's CSV-zip -> feather/ZSTD migration, just for location instead of format. Memoized
    per process (like disk_cache._read_file_cache) so file_path is only scanned once per type, not on
    every read/write.
    """
    type_dir = os.path.join(file_path, data_frame_type)
...
def _migrate_flat_files_into_type_dir(data_frame_type: str, file_path: str, type_dir: str) -> None:
    ...
    for entry in entries:
        if not pattern.match(entry):
            continue
        src = os.path.join(file_path, entry)
        if not os.path.isfile(src):
            continue
        dst = os.path.join(type_dir, entry)
        if os.path.exists(dst):
            continue
        os.rename(src, dst)
        log_i(f"disk_cache: moved {os.path.abspath(src)} into per-type cache dir {os.path.abspath(type_dir)}")
...
def cleanup_redundant_cache_files(
    ...
    for entry, start, end in files:
        ...
        if not fully_covered_by_others:
            continue
        full_path = os.path.join(type_dir, entry)

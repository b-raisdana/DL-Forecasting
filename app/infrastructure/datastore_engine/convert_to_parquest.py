...
files — the datastore_engine sub-service application.datastore_engine.convert_to_parquest orchestrates.
...
1. Legacy conversion (`find_legacy_files`/`convert_legacy_file`): the same on-touch migration
   ...
   <->convert_to_parquest cycle), exposed as an explicit batch pass over whatever legacy files remain
...
def convert_legacy_file(path: Path) -> Path | None:
    ...
    raised, so one bad file never aborts a batch — see application.datastore_engine.convert_to_parquest.
    ...
    if path.suffix == ".feather":
        try:
            ...
            log_w(f"convert_to_parquest: failed reading legacy Feather file {path}: {e}")
            return None
        ...
        try:
            ...
            log_w(f"convert_to_parquest: {path} is not a valid zip file: {e}")
            return None
...
def repair_parquet_file(path: Path, dry_run: bool = False) -> bool:
    ...
    log_i(f"convert_to_parquest: repaired date-index Parquet file {path}")
    return True

...
def fetch_ohlcv_by_range(
    ...
    response = fetch_ohlcv(
        symbol, timeframe=base_timeframe, start=start, number_of_ticks=limit, params={"timeframe": base_timeframe}
    )
    return response
        ...
        if start < datetime.utcnow().replace(tzinfo=pytz.utc):
            ...
            for i in range(20):
                try:
                    ...
                    pass
            log_i(
                f"fetch_ohlcv@{datetime.fromtimestamp(start_timestamp / 1000)}#{this_query_size}>{len(response)}",
            )

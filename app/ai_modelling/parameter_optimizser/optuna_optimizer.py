...
from ai_modelling.base import build_model, master_x_shape, pre_train_model, setup_gpu
...
DEFAULT_PARAMS = {
    "cnn_lstm_attention": dict(
        architecture="cnn_lstm_attention", cnn_filters=64, cnn_count=3, dropout_rate=0.3,
        lstm_units_1=256, lstm_units_2=128, kernel_step=2, num_heads=8, key_dim=16,
    ),
    "cnn_lstm": dict(
        architecture="cnn_lstm", cnn_filters=64, cnn_count=2, dropout_rate=0.3,
        lstm_units_1=128, lstm_units_2=64, cnn_kernel_growing_steps=2, dense_units=128,
    ),
}
...
def build_model_from_params(params: dict, y_len: int, x_shape: dict, batch_size: int = BATCH_SIZE):
    if params["architecture"] == "cnn_lstm_attention":
        model = build_cnn_lstm_attention_model(
            y_len=y_len, input_shapes=x_shape,
...
def estimate_total_budget(seconds_per_step: float, steps_per_epoch: int, epochs_per_trial: int,
                          n_trials: int, label: str = "") -> float:
...
def max_trials_for_budget(seconds_per_step: float, steps_per_epoch: int, epochs_per_trial: int,
                          budget_hours: float) -> int:
    seconds_per_trial = seconds_per_step * steps_per_epoch * epochs_per_trial
...
def objective(trial: optuna.Trial, epochs: int, steps_per_epoch: int, validation_steps: int,
              time_limit_s: float, patience: int = 5) -> float:
...
def run_study(n_trials: int = 40, timeout_hours: float | None = None, epochs: int = 30,
              steps_per_epoch: int = 50, validation_steps: int = 10, trial_time_limit_s: float = 3600,
              study_name: str = "cnn_lstm_search", storage: str | None = None) -> optuna.Study:
    sampler = optuna.samplers.TPESampler(seed=42, multivariate=True, group=True)
    pruner = optuna.pruners.HyperbandPruner(min_resource=1, max_resource=epochs, reduction_factor=3)
    study = optuna.create_study(
        direction="minimize", sampler=sampler, pruner=pruner,
        study_name=study_name, storage=storage, load_if_exists=storage is not None,
    )
    study.optimize(
        lambda trial: objective(trial, epochs=epochs, steps_per_epoch=steps_per_epoch,
                                validation_steps=validation_steps, time_limit_s=trial_time_limit_s),
        n_trials=n_trials,
...
def run_kpi_refinement(kpi_fn, base_study: optuna.Study | None = None, top_k: int = 5, n_trials: int = 20,
                       epochs: int = 30, steps_per_epoch: int = 50, validation_steps: int = 10,
                       patience: int = 5, study_name: str = "cnn_lstm_kpi_refine",
    ...
    study = optuna.create_study(
        ...
        sampler=sampler, study_name=study_name, storage=storage, load_if_exists=storage is not None,
    )
    ...
    def _objective(trial: optuna.Trial):
        ...
        model.fit(
            dataset, validation_data=dataset, epochs=epochs, steps_per_epoch=steps_per_epoch,
            validation_steps=validation_steps,
            callbacks=[tf_keras.callbacks.EarlyStopping(monitor="val_loss", patience=patience,
...
if __name__ == "__main__":
    ...
    for arch in ARCHITECTURES:
        try:
            ...
            estimate_total_budget(cost["seconds_per_step"], args.steps_per_epoch, args.epochs, args.n_trials, label=arch)
    ...
    run_study(
        n_trials=args.n_trials, timeout_hours=args.timeout_hours, epochs=args.epochs,
        steps_per_epoch=args.steps_per_epoch, validation_steps=args.validation_steps,
        trial_time_limit_s=args.trial_time_limit_min * 60, storage=args.storage,
    )

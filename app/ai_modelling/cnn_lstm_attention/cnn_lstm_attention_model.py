...
from keras.layers import Input, Conv1D, BatchNormalization, Dropout, LSTM, Dense


from keras.layers import Dense, Lambda, Softmax, Permute, Reshape
import tensorflow as tf

def multi_head_attention(inputs, num_heads=4, key_dim=16, name=None):
...
def cnn_lstm_block(name, input_shape, cnn_filters, lstm_units, cnn_count, kernel_step, dropout_rate):
    ...
    x = multi_head_attention(x, num_heads=4, key_dim=16, name=f"{name}_attention")

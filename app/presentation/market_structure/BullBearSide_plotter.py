...
def plot_single_timeframe_bull_bear_side_trends(
    single_timeframe_ohlcva: pt.DataFrame[OHLCV],
    ...
    for _start, _trend in boundaries.iterrows():
        ...
        trend_valleys = (
            valleys_only(trend_peaks_n_valleys)["low"]
            .reset_index(level="timeframe")
            .sort_index(level="date", ascending=False)
        )
        fill_color = (
            "green"
            if _trend["bull_bear_side"] == TREND.BULLISH.value
            else "red"
            if _trend["bull_bear_side"] == TREND.BEARISH.value
            else "gray"
        )
        text = bull_bear_side_repr(_start, _trend)
        name = (
            f"{_trend['bull_bear_side'].replace('_TREND', '')}: "
        ...
        if remained_number_of_scatters > 0:
            ...
            xs = (
                [_start]
                + trend_peaks.index.get_level_values("date").tolist()
                + [_trend["end"]]
                + trend_valleys.index.get_level_values("date").tolist()
            )
            ys = (
                [single_timeframe_ohlcva.loc[_start, "open"]]
                + trend_peaks["high"].tolist()
                + [single_timeframe_ohlcva.loc[_trend["end"], "close"]]
                + trend_valleys["low"].tolist()
            )
            fig.add_scatter(
                x=xs,
                y=ys,
            ...
            if "movement_start_value" in boundaries.columns:
                fig.add_scatter(
                    x=[_trend["movement_start_time"], _trend["movement_end_time"]],
...
@profile_it
def plot_multi_timeframe_bull_bear_side_trends(
    multi_timeframe_ohlcva,
    multi_timeframe_peaks_n_valleys,
    _multi_timeframe_bull_bear_side_trends,
    show: bool = True,
    save: bool = True,
    ...
    plot_multiple_figures(
        figures,
        name=f"multi_timeframe_bull_bear_side_trends."
...
def peaks_valleys_in_range(peaks_n_valleys: pd.DataFrame, start: pd.Timestamp, end: pd.Timestamp):
    return peaks_n_valleys.loc[
        (peaks_n_valleys.index.get_level_values("date") >= start)

...
def obv_macd_with_pivots(ohlcv: pt.DataFrame[OHLCV], sma_lenght=14, std_length=28, ema_lneght=1):
    t_ohlcv = ohlcv.copy()
    ...
    # 6. Slope Calculation
    len5 = 2
    t_ohlcv["Slope"] = t_ohlcv["MACD"].rolling(len5).apply(lambda x: np.polyfit(range(len(x)), x, 1)[0], raw=True)
    # 7. Pivot Detection
    xbars = 50

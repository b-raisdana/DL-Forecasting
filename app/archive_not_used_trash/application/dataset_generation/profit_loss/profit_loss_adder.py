...
from archive_not_used_trash.application.preprocessing.encoding.rolling_mean_std import reverse_rolling_mean_std
...
def max_profit_n_loss(ohlc, position_max_bars, action_delay, rolling_window):
    ...
    ohlc["worst_long_open"] = ohlc["high"].rolling(window=action_delay, min_periods=1).max().shift(1 - action_delay)
    ohlc["worst_short_open"] = ohlc["low"].rolling(window=action_delay, min_periods=1).min().shift(1 - action_delay)
    ohlc["max_high"] = ohlc["high"].rolling(rolling_window, min_periods=1).max().shift(-position_max_bars)
    ohlc["min_low"] = ohlc["low"].rolling(rolling_window, min_periods=1).min().shift(-position_max_bars)

    ohlc["max_high_distance"] = (
        ohlc["high"].rolling(rolling_window).apply(lambda x: x.argmax(), raw=True).shift(-position_max_bars)
        + action_delay
    )
    ohlc["min_low_distance"] = (
        ohlc["low"].rolling(rolling_window).apply(lambda x: x.argmin(), raw=True).shift(-position_max_bars)
        + action_delay
    )
    return ohlc
...
def quantile_maxes(ohlc, rolling_window, quantiles):
    ...
    for i in range(1, quantiles + 1):
        ...
        new_columns[f"q{i}_max_high_distance"] = (
            ohlc["high"].rolling(q_rolling_window).apply(lambda x: x.argmax(), raw=True).shift(-q_rolling_window)
        )
        new_columns[f"q{i}_min_low_distance"] = (
            ohlc["low"].rolling(q_rolling_window).apply(lambda x: x.argmin(), raw=True).shift(-q_rolling_window)
        )
...
def long_n_short_drawdown(ohlc, position_max_bars, quantiles, trigger_tf):
    ...
    ohlc.loc[not_na_indexes, "max_high_quantile"] = ohlc.loc[not_na_indexes, "max_high_distance"] / (
        position_max_bars / quantiles
    )
    ohlc.loc[not_na_indexes, "min_low_quantile"] = ohlc.loc[not_na_indexes, "min_low_distance"] / (
        position_max_bars / quantiles
    )
    ...
    ohlc["long_drawdown"] = (ohlc["worst_long_open"] - ohlc["quantile_long_min_low"]) / ohlc["atr"]
    ohlc["absolute_long_drawdown"] = ohlc["worst_long_open"] - ohlc["quantile_long_min_low"]
    ...
    ohlc["short_drawdown"] = (ohlc["quantile_short_max_high"] - ohlc["worst_short_open"]) / ohlc["atr"]
    ohlc["absolute_short_drawdown"] = ohlc["quantile_short_max_high"] - ohlc["worst_short_open"]
...
def zz_stop_loss(ohlc, windows: int, tops_percent, sl_atr_distance=2):
    ...
    ohlc["long_sl"] = np.where(
        abs(ohlc["long_sl"] - ohlc["low"]) < ohlc["atr"] * sl_atr_distance,
        ohlc["low"] - ohlc["atr"] * sl_atr_distance,
        ohlc["long_sl"],
    )
    ohlc["short_sl"] = singular_stop_loss(ohlc["high"], window=windows, tops_percent=tops_percent, mode="short")
    ohlc["short_sl"] = np.where(
        abs(ohlc["short_sl"] - ohlc["high"]) < ohlc["atr"] * sl_atr_distance,
        ohlc["high"] + ohlc["atr"] * sl_atr_distance,
        ohlc["short_sl"],
    )
...
def stop_loss(ohlc):
    ohlc["long_sl_distance"] = np.maximum(
        1, ohlc["long_drawdown"]
    )  # ohlc[['absolute_long_drawdown', 'atr']].max(axis='columns') / ohlc['atr']
    ohlc["short_sl_distance"] = np.maximum(
        1, ohlc["short_drawdown"]
    )  # ohlc[['absolute_short_drawdown', 'atr']].max(axis='columns') / ohlc['atr']
    return ohlc
...
def add_long_n_short_profit(
    ...
    ohlc = profit_n_loss(
        ...
        order_fee=order_fee,
        max_risk=max_risk,
        bar_width_risk_free_rate=risk_free_daily_rate / (timedelta(days=1) / pd.to_timedelta(trigger_tf)),
    )
...
def plot_short_profit(t):
    ...
    fig.add_scatter(
        x=t.index, y=t["max_high"], mode="lines", line=dict(color="red", width=1), name="max_high", row=2, col=1
    )
    fig.add_scatter(
        x=t.index, y=t["min_low"], mode="lines", line=dict(color="blue", width=1), name="min_low", row=2, col=1
    )
    fig.add_scatter(
        x=t.index,
        y=t["quantile_short_max_high"],
        mode="lines",
        line=dict(color="magenta", width=1),
        name="quantile_short_max_high",
        row=2,
        col=1,
    )
    fig.add_scatter(x=t.index, y=t["short_risk"], mode="lines", line=dict(color="yellow", width=1), name="short_risk")
    fig.add_scatter(
        x=t.index, y=t["short_drawdown"], mode="lines", line=dict(color="pink", width=1), name="short_drawdown"
    )
    # fig.add_scatter(x=t.index, y=t['short_profit'], mode='lines', line=dict(color='cyan', width=1),
    #                 name='short_profit')
    fig.add_scatter(
        x=t.index,
        y=t["weighted_short_profit"],
        mode="lines",
        line=dict(color="blue", width=1),
        name="weighted_short_profit",
    )
    fig.add_scatter(
        x=t.index, y=t["short_signal"], mode="lines", line=dict(color="green", width=1), name="short_signal"
    )
    fig.add_scatter(x=t.index, y=t["short_sl"], mode="lines", line=dict(color="orange", width=0.5), name="short_sl")
    fig.add_scatter(x=t.index, y=t["long_sl"], mode="lines", line=dict(color="orange", width=0.5), name="long_sl")
    fig.update_layout(
        dragmode="zoom",
...
def plot_long_profit(t):
    ...
    fig.add_scatter(
        x=t.index, y=t["max_high"], mode="lines", line=dict(color="blue", width=1), name="max_high", row=2, col=1
    )
    fig.add_scatter(
        x=t.index, y=t["min_low"], mode="lines", line=dict(color="red", width=1), name="min_low", row=2, col=1
    )
    fig.add_scatter(
        x=t.index,
        y=t["quantile_long_min_low"],
        mode="lines",
        line=dict(color="magenta", width=1),
        name="quantile_long_min_low",
        row=2,
        col=1,
    )
    fig.add_scatter(x=t.index, y=t["long_risk"], mode="lines", line=dict(color="yellow", width=1), name="long_risk")
    fig.add_scatter(
        x=t.index, y=t["long_drawdown"], mode="lines", line=dict(color="pink", width=1), name="long_drawdown"
    )
    # fig.add_scatter(x=t.index, y=t['long_profit'], mode='lines', line=dict(color='cyan', width=1),
    #                 name='long_profit')
    fig.add_scatter(
        x=t.index,
        y=t["weighted_long_profit"],
        mode="lines",
        line=dict(color="blue", width=1),
        name="weighted_long_profit",
    )
    fig.add_scatter(x=t.index, y=t["long_signal"], mode="lines", line=dict(color="green", width=1), name="long_signal")
    fig.update_layout(
        dragmode="zoom",
        yaxis2=dict(fixedrange=False),
        margin=dict(l=0, r=0, t=10, b=10),
...
def zz_plot_long_n_short_profit(t, samples=[]):
    ...
    fig.add_scatter(
        x=t.index, y=t["long_sl"], mode="lines", line=dict(color="orange", width=1), name="long_sl", row=2, col=1
    )
    fig.add_scatter(
        x=t.index, y=t["short_sl"], mode="lines", line=dict(color="orange", width=1), name="short_sl", row=2, col=1
    )
    fig.add_trace(
        go.Candlestick(x=t.index, open=t["open"], close=t["close"], high=t["high"], low=t["low"]), row=2, col=1
    )
    ...
    fig.update_layout(
        dragmode="zoom",
        yaxis2=dict(title="Price", fixedrange=False),
        margin=dict(l=0, r=0, t=10, b=10),
    ).show()
    return fig
...
def add_scaled_scatter(fig, t, name, color, fill_na=0):
    ...
    fig.add_scatter(
        x=t.index, y=t[name] * float(sc), mode="lines", line=dict(color=color, width=1), name=f"{name}{sc_mark}"
    )
...
def zz_plot_mt_train_n_test(x, y, n, base_ohlcv, show=True):
    ...
    double_trace = go.Candlestick(
        x=reconstructed_double.index.get_level_values(level="date"),
    ...
    trigger_trace = go.Candlestick(
        x=reconstructed_trigger.index.get_level_values(level="date"),
    ...
    pattern_trace = go.Candlestick(
        x=reconstructed_pattern.index.get_level_values(level="date"),
    ...
    structure_trace = go.Candlestick(
        x=reconstructed_structure.index.get_level_values(level="date"),
    ...
    y_trace = go.Candlestick(
        x=y[n].index.get_level_values(level="date"),
        open=y[n]["open"],
        high=y[n]["high"],
        low=y[n]["low"],
        close=y[n]["close"],
    ...
    ohlcv_trace = go.Candlestick(
        x=ohlcv_slice.index.get_level_values(level="date"),
    ...
    fig = go.Figure(
        data=[double_trace, trigger_trace, pattern_trace, structure_trace, y_trace, ohlcv_trace], layout=layout
    )
...
def tops_mean(series: pd.Series, window, mode: Literal["smallest", "largest"], percent):
    ...
    return series.rolling(window).apply(
        lambda x: np.mean(sorted(x, reverse=sort_to_reverse_map[mode])[:top_k]), raw=True
    )

...
def npz_file_dataset(cache_folder, poll_interval_sec=1.0):
    ...
    return tf.data.Dataset.from_generator(file_gen, output_signature=tf.TensorSpec(shape=(), dtype=tf.string))
...
def load_and_delete_npz(path_tensor):
    ...
    raw = tf.py_function(func=_load_npz, inp=[path_tensor], Tout=out_types)
                    ...
                    for _ in range(100):
                        try:
                            ...
                            try:
                                ...
                                log_i(f"Generated cache file: {os.path.basename(final_path)} (batch size={len(ys)})")
                            except Exception as e:

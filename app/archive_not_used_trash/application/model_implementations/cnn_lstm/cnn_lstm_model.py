import numpy as np
from helper.logging.do_log import log_e
...
class CNNLSTMModel(tf_keras.models.Model):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(name="CNNLSTM_Model", **kwargs)
        self.y_len = y_len
        ...
        self.submodels = {
            key: CNNLSTMLayer(
                model_prefix=f"{key}_cnn_lstm_layer",
...
class CNNLSTMLayer(tf_keras.layers.Layer):
    def __init__(
        ...
        super().__init__(name=f"{model_prefix}_layer")
        ...
        for i in range(cnn_count):
            ...
            self.bn_layers.append(tf_keras.layers.BatchNormalization(name=f"{model_prefix}_batch_norm_conv{i + 1}"))
        ...
        for i, lstm_units in enumerate(lstm_units_list):
            ...
            self.lstm_layers.append(
                tf_keras.layers.LSTM(
                    lstm_units,
                    return_sequences=return_seq,  # dtype='float32',
        ...
        self.output_layer = tf_keras.layers.Dense(
            np.prod(output_shape), activation="linear", name=f"{model_prefix}_output", dtype="float32"
        )

...
def choose_dataset_files(x_shape: dict[str, tuple[int, int]]) -> tuple[str, str, str]:
    ...
    batch_files = sorted(f for f in os.listdir(folder_path) if f.startswith("dataset-") and f.endswith(".zip"))
    val_files = sorted(f for f in os.listdir(folder_path) if f.startswith("validators-") and f.endswith(".zip"))
...
def predict_once(x_shape: dict[str, tuple[int, int]]) -> None:
    ...
    * Runs the model and prints prediction vs. ground-truth
    ...
    df = pd.DataFrame(preds, columns=["p_short_s", "p_long_s"], dtype=np.float32)
    df["y_short_s"] = ys[:, 0]
    df["y_long_s"] = ys[:, 1]
    ...
    df.loc[
        (
            (df["ydf_long_s"] > df["ydf_short_s"]) & (df["p_long_s"] > df["p_short_s"])
            | (df["ydf_long_s"] < df["ydf_short_s"]) & (df["p_long_s"] < df["p_short_s"])
        ),
        "final",
    ] = True

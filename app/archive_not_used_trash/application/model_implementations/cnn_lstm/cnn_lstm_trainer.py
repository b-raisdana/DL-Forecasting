...
from archive_not_used_trash.application.dataset_generation.feeders.npz_batch import build_npz_dataset
from archive_not_used_trash.application.model_implementations.cnn_lstm.cnn_lstm_model import CNNLSTMLayer, CNNLSTMModel
...
def train_model(
    train_dataset: Dataset | Generator,
    val_dataset: Dataset | Generator,
    ...
    pre_train_model()
    model_name = (
        f"cnn_lstm_attention.mt_pnl_n_ind"
    ...
    if model is None:
        ...
        if not rebuild_model and os.path.exists(model_path_keras):
            ...
            model = tf_keras.models.load_model(
                model_path_keras, custom_objects={"CNNLSTMModel": CNNLSTMModel, "CNNLSTMLayer": CNNLSTMLayer}
            )
            ...
            model = CNNLSTMModel(
                ...
                cnn_count=cnn_count,
                cnn_kernel_growing_steps=cnn_kernel_growing_steps,
    ...
    # Train the model
    early_stopping: callable = tf_keras.callbacks.EarlyStopping(
...
if __name__ == "__main__":
    ...
    try:
        while True:  # infinite loop
            ...
            p = mp.Process(target=run_trainer, args=(training_round_counter,))  # 1️⃣ start a brand‑new process
            p.start()

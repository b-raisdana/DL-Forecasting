...
import pandas as pd
import pytz
from archive_not_used_trash.application.backtesting.BaseTickStructure import BaseTickStructure
from archive_not_used_trash.application.backtesting.order_helper import (
    add_order_info,
    dict_of_order,
    order_is_closed,
    order_is_open,
...
class ExtendedStrategy(bt.Strategy):  # type: ignore[misc]  # backtrader ships no type stubs
    signal_df: pt.DataFrame[SignalDFM] = None  # type: ignore[assignment]  # sentinel default, set in __init__
    orders_df: pd.DataFrame = None  # type: ignore[assignment]  # sentinel default, set in __init__
    vault_df: pd.DataFrame = None  # type: ignore[assignment]  # sentinel default, set in __init__

    multi_timeframe_ohlcva: pt.DataFrame[MultiTimeframeOHLCVA] = None  # type: ignore[assignment]  # sentinel default
    ...
    order_groups_info: dict[int, dict[str, object]] = {}
    broker_initial_cash: float | None = None

    def get_order_groups(self) -> dict[int, tuple[bt.Order, bt.Order, bt.Order]]:
        result = {
            order_group_id: (
                self.original_orders[order_group_id],
                self.sl_orders[order_group_id],
                self.tp_orders[order_group_id],
            )
            for order_group_id in self.original_orders
        }
        return result

    @profile_it  # type: ignore[untyped-decorator]  # profile_it is an untyped decorator (helper.logging)
    def __init__(self) -> None:
        self.signal_df = SignalDf.new()  # type: ignore[assignment]  # ExtendedDf.new() returns the base schema type
    ...
    def start(self) -> None:
        self.broker_initial_cash: float = float(self.broker.get_cash())

    def new_order_group_id(self) -> int:
        return len(self.order_groups_info)

    def add_signal_source_data(self) -> None:
        raise NotImplementedError

    @staticmethod
    def risk_size_sizer(limit_price: float, sl_price: float, size: float | None = None) -> tuple[float, float]:
        """
        ...
        - size (float, optional): The desired position size. If not provided, it will be calculated based on risk
             percentage.
    ...
    def post_bracket_order(
        ...
        if app_config.check_assertions and (
            order_group_id in self.original_orders
            or order_group_id in self.sl_orders
            or order_group_id in self.tp_orders
        ):
            raise AssertionError("order_group_id is not unique!")
    ...
    def spent_money(self) -> float:
        ...
        return float(self.broker_initial_cash - self.broker.get_cash())

    def allocate_order_cash(self, limit_price: float, sl_price: float, size: float | None = None) -> float:
    ...
    def set_date_range(self, date_range_str: str | None = None) -> None:
    ...
    def get_order_group(self, order: bt.Order) -> tuple[bt.Order, bt.Order, bt.Order]:
        ...
        if app_config.check_assertions and order not in [original_order, stop_order, profit_order]:
            raise AssertionError(
                f"The order: should be one of group members O:{original_order} S:{stop_order} P:{profit_order}"
            )
        return original_order, stop_order, profit_order

    def next_log(self, force: bool = False) -> None:
        if force or self.next_runs % 100 == 0:
            log_d(f"Ran {self.next_runs} Next()s reached {self.candle().date}")
        self.next_runs += 1

    def movement_intersect(self, target_low: float, target_high: float) -> float:
    ...
    def verify_triple_oder_status(self) -> None:
        """
        check:
    ...
    def stop(self) -> None:
        ...
        if self.vault_df is not None:
            self.vault_df.to_csv(
                os.path.join(
                    symbol_data_path(), f"{self.__class__.__name__}.vault.{app_config.id}.{self.date_range_str}.csv"
                )
            )
        if self.orders_df is not None:
            self.orders_df.to_csv(
                os.path.join(
                    symbol_data_path(), f"{self.__class__.__name__}.orders.{app_config.id}.{self.date_range_str}.csv"
                )
            )
        if self.signal_df is not None:
            self.signal_df.to_csv(
                os.path.join(
                    symbol_data_path(), f"{self.__class__.__name__}.signals.{app_config.id}.{self.date_range_str}.csv"
                )
            )
        if self.original_orders is not None and len(self.original_orders) > 0:
            df = pd.DataFrame()
            for index in self.original_orders:
    ...
    def log_order(self, order: bt.Order, index: int | None = None) -> None:
        if index is None:
            index = order.ref
        order_dict = cast(dict[str, object], dict_of_list(dict_of_order(order)))
    ...
    def log_vault(self, save: bool = True) -> None:
        _dict = self.get_vault()
        self.vault_df = concat(
            self.vault_df, pd.DataFrame(_dict, index=[self.candle().date])
    ...
    def get_vault(self) -> dict[str, object]:
        ...
        return cast(dict[str, object], dict_of_list(_dict))
    ...
    def notify_order(self, order: bt.Order) -> None:
        ...
        if app_config.check_assertions and not (
            order_name(order) in self.signal_df["original_order_id"].dropna().tolist()
            or order_name(order) in self.signal_df["stop_loss_order_id"].dropna().tolist()
            or order_name(order) in self.signal_df["take_profit_order_id"].dropna().tolist()
        ):
        ...
        if order.status in [order.Submitted, order.Accepted]:
            ...
            log_w(f"Order:{order_name(order)} Partially executed")
            # raise NotImplementedError
        ...
        if order.info["custom_type"] in [
            BracketOrderType.StopLoss.value,
            BracketOrderType.TakeProfit.value,
        ] and order.status in [order.Completed, order.Partial]:
    ...
    def ordered_signals(self, signal_df: SignalDf | None = None) -> pt.DataFrame[SignalDFM]:
        raise NotImplementedError
        # if signal_df is None:
        #     signal_df = self.signal_df
        #     if not isinstance(signal_df, SignalDf):
        #         raise TypeError(f"signal_df must be a SignalDf, got {type(signal_df)}")
    ...
    def active_signals(self) -> pt.DataFrame[SignalDFM]:
        ...
        result = self.signal_df[
            ...
            & (
                ...
                (self.signal_df["end"].isna()) | (self.signal_df["end"] > self.candle().date)
            )
            & (self.signal_df["order_is_active"].isna() | ~self.signal_df["order_is_active"])
        ]
        return result

    def execute_active_signals(self) -> None:
        ...
        for idx in range(len(_executable_signals)):
            ...
            if pd.notna(_executable_signals.iloc[idx]["take_profit"]):
                original_order: bt.Order
                # try:
                size = self.allocate_order_cash(
                    _executable_signals.iloc[idx]["limit_price"], signal["stop_loss"]
                )  # signal['base_asset_amount'])
                if size <= 0:
                    log_w(f"Size is {size} @ {self.candle().date}")
                    continue
                if pd.notna(signal["limit_price"]) and pd.notna(signal["stop_loss"]):
                    if (
                        pd.isna(signal["trigger_price"])
                        or ((signal_side == OrderSide.Buy.value) and (self.candle().high > signal["trigger_price"]))
                        or ((signal_side == OrderSide.Sell.value) and (self.candle().low < signal["trigger_price"]))
                    ):
                        original_order, stop_order, profit_order = self.bracket_executors[signal_side](
                            size=size,
                ...
                log_d(
                    f"Signal:{SignalDf.to_str(signal_index, signal)} Ordered: "  # type: ignore[arg-type]

...
import pandas as pd
from archive_not_used_trash.application.backtesting.ExtendedStrategy import ExtendedStrategy
...
class BasePatternStrategy(ExtendedStrategy):
    ...
    def add_signal_source_data(self) -> None:
        base_patterns = get_multi_timeframe_base_patterns(self.date_range_str)
        ...
        if order.status == bt.Order.Completed:
            if order.info["custom_type"] == BracketOrderType.StopLoss.value:
                ...
                if app_config.check_assertions and (
                    not original_order.status == bt.Order.Completed or not profit_order.status == bt.Order.Canceled
                ):
                    raise AssertionError(
                        "(not original_order.status == bt.Order.Completed or "
                ...
                log_d(
                    f"repeating Signal according to Stop-Loss on {SignalDf.to_str(original_signal_index, signal)} @ "
                    f"{self.candle()}"
                )
                ...
                if app_config.check_assertions and str(self.signal_df.index.names) != str(
                ...
                new_index = (
                    self.candle().date,
                    original_signal_index[1],
                    original_signal_index[2],
                    original_signal_index[3],
                )
                self.signal_df.loc[new_index] = repeat_signal
                if not self.signal_df.index.is_unique:
                    pass
                log_d(f"repeated Signal:{SignalDf.to_str(new_index, repeat_signal)}@{self.candle().date} ")  # type: ignore[arg-type]
                ...
                original_order, stop_order, profit_order = self.get_order_group(order)
                if not original_order.status == bt.Order.Completed or not stop_order.status == bt.Order.Canceled:
    ...
    def overlapping_base_patterns(
        self, base_patterns: pt.DataFrame[MultiTimeframeBasePattern] | None = None
    ...
    def no_signal_active_base_patterns(
        self, band: Literal["upper", "below"], base_patterns: pt.DataFrame[MultiTimeframeBasePattern] | None = None
    ) -> pt.DataFrame[MultiTimeframeBasePattern]:
        if base_patterns is None:
            base_patterns = self.base_patterns
    ...
    def is_trading_fee_reasonable(self, limit_price: float, take_profit: float) -> bool:
        commission_rate = self.broker.getcommissioninfo()["commission"]
        average_cost_of_trade = ((app_config.base_pattern_risk_reward_rate + 1) * commission_rate) * limit_price
        return not abs(take_profit - limit_price) < average_cost_of_trade * app_config.trading_fee_safe_side_multiplier

    def add_signal(
        self,
        base_pattern_timeframe: str,
        base_pattern_date: datetime,
        base_pattern: pt.Series[MultiTimeframeBasePattern],
        band: Literal["upper", "below"],
    ) -> pt.DataFrame[SignalDFM]:
        ...
        limit_price = opr(
            base_pattern[f"internal_{high_low}"],
            base_pattern["atr"] * app_config.base_pattern_order_limit_price_margin_percentage,
        )
        stop_loss = base_pattern[f"internal_{reverse_high_low}"]
        take_profit = opr(base_pattern[f"internal_{high_low}"], base_length * app_config.base_pattern_risk_reward_rate)
        ...
        effective_end = base_pattern["end"] if pd.notna(base_pattern["end"]) else base_pattern["ttl"]
        if app_config.check_assertions and pd.isna(effective_end):
        ...
        self.signal_df = SignalDf.concat(self.signal_df, new_signal)  # type: ignore[assignment, arg-type]
        log_d(
            f"added Signal {SignalDf.to_str(new_signal.index[0], new_signal.iloc[0])} @ {self.candle().date}"  # type: ignore[arg-type]
        )
        return self.signal_df

    def candle_overlaps_base(self, base_pattern: pt.Series[BasePattern]) -> bool:
        return self.movement_intersect(base_pattern["internal_low"], base_pattern["internal_high"]) > 0
    ...
    def extract_signals(self) -> None:
        ...
        if len(upper_overlapping_base_patterns) > 0:
            for index, row in upper_overlapping_base_patterns.iterrows():
                timeframe, start = cast(tuple[str, pd.Timestamp], index)
                base_pattern = cast(pt.Series[MultiTimeframeBasePattern], row)
                # if self.candle_overlaps_base(base_pattern):
                self.add_signal(timeframe, start, base_pattern, band="upper")
                if app_config.check_assertions and pd.notna(
                    self.base_patterns.loc[(timeframe, start), "upper_band_signal_generated"]
                ):
        ...
        if len(below_overlapping_base_patterns) > 0:
            for index, row in below_overlapping_base_patterns.iterrows():
                timeframe, start = cast(tuple[str, pd.Timestamp], index)
                base_pattern = cast(pt.Series[MultiTimeframeBasePattern], row)
    ...
    def stop(self) -> None:
        _multi_timeframe_ohlcva = get_multi_timeframe_ohlcva(self.date_range_str)
...
def test_strategy(cash: float, date_range_str: str | None = None) -> None:
    ...
    raw_data = get_base_timeframe_ohlcv(date_range_str)
    data = bt.feeds.PandasData(
        dataname=raw_data, datetime=None, open=0, close=1, high=2, low=3, volume=4, openinterest=-1
    )
    ...
    log_d(f"Starting Portfolio Value: {cerebro.broker.getvalue():.2f}")
    cerebro.run()

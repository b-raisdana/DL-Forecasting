...
def plot_multi_timeframe_ohlcva(multi_timeframe_ohlcva, name: str = "", show: bool = True, save: bool = True) -> None:
    ...
    for _, timeframe in enumerate(app_config.timeframes):
        figures.append(
            plot_ohlcva(
                single_timeframe(multi_timeframe_ohlcva, timeframe), show=False, save=False, name=f"{timeframe} ohlcva"
            )
        )
    plot_multiple_figures(
        figures, name=f"multi_timeframe_ohlcva.{file_id(multi_timeframe_ohlcva, name)}", save=save, show=show
    )
...
def plot_multi_timeframe_ohlcv(multi_timeframe_ohlcv, date_range_str, show: bool = True, save: bool = True):
    ...
    for _, timeframe in enumerate(app_config.timeframes):
        figures.append(
            plot_ohlcv(
                single_timeframe(multi_timeframe_ohlcv, timeframe), show=False, save=False, name=f"{timeframe} ohlcv"
            )
        )
...
@profile_it
def plot_ohlcv(
    ohlcv: pd = pd.DataFrame(columns=["open", "high", "low", "close"]),
    ...
    fig = (
        ...
        .update_layout(yaxis_title=name)
    )
    if base_ohlcv is not None:
        fig.add_candlestick(
            x=base_ohlcv.index.values,
            open=base_ohlcv["open"],
            high=base_ohlcv["high"],
            low=base_ohlcv["low"],
...
@profile_it
def plot_ohlcva(
    ohlcva: pd.DataFrame, save: bool = True, show: bool = True, name: str = "", base_ohlcv: pt.DataFrame[OHLCV] = None
) -> plgo.Figure:
    """
    ...
    fig = add_atr_scatter(
        fig, ohlcva.index, midpoints=midpoints, widths=CandleSize.Spinning.value.max * ohlcva["atr"], name="Standard"
    )
    fig = add_atr_scatter(
        fig, ohlcva.index, midpoints=midpoints, widths=CandleSize.Standard.value.max * ohlcva["atr"], name="Long"
    )
    fig = add_atr_scatter(
        fig, ohlcva.index, midpoints=midpoints, widths=CandleSize.Long.value.max * ohlcva["atr"], name="Spike"
    )
...
def plot_ohlcva_with_subplot(
    ...
    sub_fig = add_atr_scatter(
        sub_fig, ohlcva.index, midpoints=midpoints, widths=CandleSize.Standard.value.max * ohlcva["atr"], name="Long"
    )
    sub_fig = add_atr_scatter(
        sub_fig, ohlcva.index, midpoints=midpoints, widths=CandleSize.Long.value.max * ohlcva["atr"], name="Spike"
    )
...
def plot_merged_timeframe_ohlcva(multi_timeframe_ohlcva: pt.DataFrame[MultiTimeframeOHLCVA]):
    ...
    timeframe_list = [
        timeframe for timeframe in app_config.timeframes if timeframe in multi_timeframe_ohlcva_timeframes
    ]
    for timeframe in timeframe_list:
        ohlcva = single_timeframe(multi_timeframe_ohlcva, timeframe)
        fig.add_candlestick(
            x=ohlcva.index.values,
            open=ohlcva["open"],
            high=ohlcva["high"],
            low=ohlcva["low"],
            close=ohlcva["close"],

...
def test_bull_bear_side_trends(trends: pt.DataFrame[BullBearSide]):
    for start, trend in trends.iterrows():
        ...
        trend_overlaps = trends[(trends.index <= end) & (trends["end"] >= start)]
        if len(trend_overlaps) > 0:
            ...
            if direction in trend_overlaps["bull_bear_side"]:
                ...
                _msg = str.join(
                    [
                        bull_bear_side_repr(o_start, o_trend)
                        for o_start, o_trend in trend_overlaps[trend_overlaps["bull_bear_side"] == direction].iterrows()
                    ]
                )
            ...
            start_overlaps = trends[(trends.index <= end) & (trends.index >= start)]
            end_overlaps = trends[(trends["end"] <= end) & (trends["end"] >= start)]
            if len(start_overlaps) > 0:
                if len(start_overlaps) > 1:
                    raise Exception(
                        f"{bull_bear_side_repr(start, trend)} have more than one start overlaps:"
                        f"{[bull_bear_side_repr(o_start, o_trend) for o_start, o_trend in start_overlaps]}"
                    )
                if start != start_overlaps.iloc[0]["end"]:
                    raise Exception(
                        f"start of {bull_bear_side_repr(start, trend)} does not match with end of "
                        f"{bull_bear_side_repr(start_overlaps.index[0], start_overlaps.iloc[0])}"
                    )
            if len(end_overlaps) > 0:
                if len(end_overlaps) > 1:
                    raise Exception(
                        f"{bull_bear_side_repr(start, trend)} have more than one end overlaps:"
                        f"{[bull_bear_side_repr(o_start, o_trend) for o_start, o_trend in end_overlaps]}"
                    )
                if trend["end"] != end_overlaps.index[0]:
                    raise Exception(
                        f"end of {bull_bear_side_repr(start, trend)} does not match with start of "
                        f"{bull_bear_side_repr(end_overlaps.index[0], end_overlaps.iloc[0])}"
                    )


def test_side_trend_not_overlapping_another_side(
    start: datetime, trend: pd.Series, overlapping_trends: pt.DataFrame[BullBearSide], raise_exception: bool = False
):

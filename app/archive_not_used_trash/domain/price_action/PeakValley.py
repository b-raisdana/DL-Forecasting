...
from helper.data_preparation import after_under_process_date, concat, trim_to_date_range
from archive_not_used_trash.helper.data_preparation import expand_date_range, nearest_match
...
def calculate_strength(
    peaks_or_valleys: pt.DataFrame[PeakValley], top_type: TopTYPE, ohlcv: pt.DataFrame[OHLCV]
    ...
    peaks_or_valleys["strength"] = app_config.INFINITY_TIME_DELTA
...
def insert_distance(
    base: pt.DataFrame[PeakValley], target: pt.DataFrame[OHLCV], top_type: TopTYPE, direction: Literal["right", "left"]
) -> pt.DataFrame[PeakValley]:
    """
    ...
    if top_type == TopTYPE.PEAK:  # todo: test
        ...
        base = insert_crossing2(
            ...
            target=target,
            direction=direction,
            base_target_column="low",
            target_compare_column="low",
            more_significant=lambda target, base: target < base,
        )
    valid_crossing_times = base[base[f"{direction}_crossing_time"].notna()].index
    base.loc[valid_crossing_times, direction + "_distance"] = abs(
        pd.to_datetime(valid_crossing_times) - base.loc[valid_crossing_times, f"{direction}_crossing_time"]
    )
    return base


def insert_crossing2(
    base: pd.DataFrame,
    target: pd.DataFrame,
    ...
    drop_base_without_crossing2(
        bases_to_compare=remained_bases,
        base_target_column=base_target_column,
        target=target,
        more_significant=more_significant,
        direction=direction,
    ...
    while len(remained_bases) > 0:
        ...
        bases_to_compare["target_index"] = nearest_match(
            bases_to_compare.index.get_level_values("date"), target.index, direction=direction, shift=0
        )
        ...
        while number_of_crossed_bases > 0 and len(bases_to_compare) > 0:
            ...
            crossed_bases, target = find_crossing_single_iteration(
                bases_to_compare,
                base_target_column,
                target,
                target_compare_column,
...
def drop_base_without_crossing(bases_to_compare, target, base_target_column, base_type, cross_direction, direction):
    n = len(target)
    if (base_type == TopTYPE.PEAK and cross_direction == "out") or (
        base_type == TopTYPE.VALLEY and cross_direction == "in"
    ):
        # target_compare_column == 'high':
        if direction == "left":
            target["max_value"] = (
...
def drop_base_without_crossing2(
    bases_to_compare,
    target,
    base_target_column,
    more_significant,
    direction: Literal["right", "left"],
    target_compare_column: Literal["low", "high"],
):
    n = len(target)
    ...
    if more_significant(target=2, base=1):
        if direction == "left":
            ...
            target["max_value"] = target["max_value"].shift(-1)
        bases_to_compare.loc[bases_to_compare["target_index"].notna(), "target_max_value"] = target.loc[
            bases_to_compare["target_index"].dropna(), "max_value"
        ].tolist()
        without_crossings = bases_to_compare[
...
def insert_crossing_info(base, crossed_bases, direction, target, target_compare_column):
    ...
    base.loc[crossed_bases.index, f"{direction}_crossing_value"] = target.loc[
        crossed_bases["target_date"].tolist(), target_compare_column
    ].to_list()
    # return base
...
def find_crossing_single_iteration(
    bases_to_compare,
    base_compare_column,
    target,
    target_compare_column,
    ...
    bases_shall_have_crossing: bool = False,
):
    """
    ...
    adjacent_target_dates = nearest_match(needles=base_dates, reference=target_dates, direction=direction)
    ...
    bases_with_unique_adjacent_target = bases_with_adjacent_target[
        ~(bases_with_adjacent_target["adjacent_target_date"].duplicated(keep="first"))
    ]
    adjacent_target_dates = bases_with_unique_adjacent_target["adjacent_target_date"].tolist()
    ...
    target.loc[adjacent_target_dates, reverse + "_base_target"] = bases_with_unique_adjacent_target[
        base_compare_column
    ].tolist()
    # propagate
    if direction == "right":
...
def find_peaks_n_valleys(
    base_ohlcv: pd.DataFrame, sort_index: bool = True
) -> pt.DataFrame[PeakValley]:  # , max_cycles=100):
...
@profile_it
def major_timeframe(multi_timeframe_df: pd.DataFrame, timeframe: str) -> pt.DataFrame[MultiTimeframePeakValley]:
    """
...
def multi_timeframe_peaks_n_valleys(expanded_date_range: str) -> pt.DataFrame[MultiTimeframePeakValley]:
    base_ohlcv = get_base_timeframe_ohlcv(expanded_date_range)
    ...
    _peaks_n_valleys = _peaks_n_valleys.set_index(
        "timeframe",
        append=True,
    )
    ...
    _peaks_n_valleys = cast_and_validate(
        _peaks_n_valleys, MultiTimeframePeakValley, zero_size_allowed=after_under_process_date(expanded_date_range)
    )
...
def get_multi_timeframe_peaks_n_valleys(date_range_str: str = None) -> pt.DataFrame[MultiTimeframePeakValley]:
    ...
    expanded_date_range = expand_date_range(
        date_range_str, time_delta=4 * pd.to_timedelta(biggest_timeframe), mode="both"
    )
    ...
    _peaks_n_valleys = _peaks_n_valleys[
        ["open", "high", "low", "close", "volume", "peak_or_valley", "strength", "permanent_strength"]
    ]
    return _peaks_n_valleys


# @measure_time
def calculate_strength_of_peaks_n_valleys(
    time_ohlcv: pt.DataFrame[OHLCV], time_peaks_n_valleys: pt.DataFrame[PeakValley]
) -> pt.DataFrame[PeakValley]:
...
def old_insert_previous_n_next_top(top_type: TopTYPE, peaks_n_valleys, ohlcv):
    if "previous_top_index" not in ohlcv.columns:
        ohlcv["previous_top_index"] = None
    if "previous_top_value" not in ohlcv.columns:
        ohlcv["previous_top_value"] = None
    if "next_top_index" not in ohlcv.columns:
        ohlcv["next_top_index"] = None
    if "next_top_value" not in ohlcv.columns:
        ohlcv["next_top_value"] = None
    ...
    for i in range(len(tops)):
        if i == len(tops) - 1:
            ...
            is_previous_for_indexes = ohlcv.loc[
                (ohlcv.index > tops.index.get_level_values("date")[i])
                & (ohlcv.index <= tops.index.get_level_values("date")[i + 1])
            ].index
        if i == 0:
            is_next_for_indexes = ohlcv.loc[ohlcv.index <= tops.index.get_level_values("date")[i]].index
        else:
            is_next_for_indexes = ohlcv.loc[
                (ohlcv.index > tops.index.get_level_values("date")[i - 1])
                & (ohlcv.index <= tops.index.get_level_values("date")[i])
            ].index
        ohlcv.loc[is_previous_for_indexes, "previous_top_index"] = tops.index.get_level_values("date")[i]
        ohlcv.loc[is_previous_for_indexes, "previous_top_value"] = tops.iloc[i][high_or_low]
        ohlcv.loc[is_next_for_indexes, "next_top_index"] = tops.index.get_level_values("date")[i]
        ohlcv.loc[is_next_for_indexes, "next_top_value"] = tops.iloc[i][high_or_low]
    return ohlcv


@profile_it
def insert_previous_n_next_top(
    top_type, peaks_n_valleys: pt.DataFrame[PeakValley], ohlcv: pt.DataFrame[OHLCV]
) -> pt.DataFrame[OHLCV]:
    # Define columns
    ...
    previous_tops = pd.DataFrame(
        index=tops.index.get_level_values("date").shift(1, app_config.timeframes[0])
    ).sort_index()
    ...
    ohlcv = pd.merge_asof(
        ohlcv.sort_index(), previous_tops, left_index=True, right_index=True, direction="backward", suffixes=("_x", "")
    )
    ...
    # merge the previous and next values
    ohlcv = pd.merge_asof(
        ohlcv.sort_index(), next_tops, left_index=True, right_index=True, direction="forward", suffixes=("_x", "")
    )

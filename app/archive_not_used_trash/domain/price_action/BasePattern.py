...
def add_candle_size(ohlcva: pt.DataFrame[OHLCVA]) -> pt.DataFrame[OHLCVA]:
    ...
    ohlcva.loc[
        (CandleSize.Standard.value.min < ohlcva["atr_ratio"]) & (ohlcva["atr_ratio"] <= CandleSize.Standard.value.max),
        "size",
    ] = CandleSize.Standard.name
    ohlcva.loc[
        (CandleSize.Long.value.min < ohlcva["atr_ratio"]) & (ohlcva["atr_ratio"] <= CandleSize.Long.value.max), "size"
    ] = CandleSize.Long.name
    ohlcva.loc[(CandleSize.Spike.value.min < ohlcva["atr_ratio"]), "size"] = CandleSize.Spike.name
    return ohlcva


def sequence_of_spinning(
    ohlcva: pt.DataFrame[OHLCVA], timeframe: str, number_of_base_spinning_candles: int = None
) -> pt.Index[Timestamp]:
    """
    ...
    if len(ohlcva) < number_of_base_spinning_candles + 1:
        raise ValueError(
            f"Insufficient data ({len(ohlcva)}) "
    ...
    spinning_candles = (
        ohlcva.iloc[number_of_base_spinning_candles + loop_shift - 1 :]
        .loc[ohlcva["size"] == CandleSize.Spinning.name]
        .copy()
    )
...
def base_from_sequence(
    _sequence_of_spinning: pt.DataFrame[OHLCVA],
    ohlcva: pt.DataFrame[OHLCVA],
    number_of_base_spinning_candles: int,
    timeframe: str,
    ...
    overlap_columns, _sequence_of_spinning = add_previous_candle_overlap(
        _sequence_of_spinning, ohlcva, number_of_base_spinning_candles, timeframe
    )
...
def add_previous_candle_overlap(_sequence_of_spinning, ohlcva, number_of_base_spinning_candles, timeframe):
    ...
    for i in range(loop_range_shift, number_of_base_spinning_candles + loop_range_shift):
        ...
        _sequence_of_spinning.loc[
            _sequence_of_spinning[f"previous_candle_intersect_length_{i}"] == 0, f"previous_candle_overlap_{i}"
        ] = 0
...
def timeframe_base_pattern(
    ...
    timeframe_base_patterns = base_from_sequence(
        _sequence_of_spinning, ohlcva, number_of_base_spinning_candles, timeframe
    )
    ...
    timeframe_base_patterns = pd.merge_asof(
        left=timeframe_base_patterns,
        right=a_pattern_ohlcva[["a_pattern_atr"]],
        left_index=True,
        right_index=True,
        direction="backward",
    )
    ...
    timeframe_base_patterns = pd.merge_asof(
        left=timeframe_base_patterns,
        right=a_trigger_ohlcva[["a_trigger_atr"]],
        left_index=True,
        right_index=True,
        direction="backward",
    )
...
def set_zero_trigger_candle_internal_high_and_lows(
    ...
    for i in range(loop_shift, number_of_base_spinning_candles + loop_shift):
        overlapped_on_nth_column = zero_trigger_candle_patterns[
            zero_trigger_candle_patterns["min_intersect_length"] == f"previous_candle_intersect_length_{i}"
        ].index
        zero_trigger_candle_patterns.loc[overlapped_on_nth_column, "internal_low"] = zero_trigger_candle_patterns[
            f"previous_candle_intersect_low_{i}"
        ]
        zero_trigger_candle_patterns.loc[overlapped_on_nth_column, "internal_high"] = zero_trigger_candle_patterns[
            f"previous_candle_intersect_high_{i}"
        ]
...
def update_zero_trigger_candles(
    timeframe_base_patterns: pt.DataFrame["BasePattern"], number_of_base_spinning_candles: int
) -> pt.DataFrame["BasePattern"]:
    ...
    timeframe_base_patterns = set_zero_trigger_candle_internal_high_and_lows(
        timeframe_base_patterns, number_of_base_spinning_candles
    )
    return timeframe_base_patterns


def verify_backtesting_ability_by_base_timeframe_atr(
    base_patterns: pt.DataFrame[MultiTimeframeBasePattern],
    expanded_multi_timeframe_ohlcva: pt.DataFrame[MultiTimeframeOHLCVA],
):
    ...
    base_patterns["ignore_backtesting"] = base_patterns["size"] < (
        base_patterns["base_timeframe_atr"] * app_config.base_pattern_small_to_trace_in_base_candles_atr_factor
    )
    return base_patterns


def multi_timeframe_base_patterns(
    expanded_multi_timeframe_ohlcva: pt.DataFrame[MultiTimeframeOHLCVA], timeframe_shortlist: list["str"] = None
    ...
    for timeframe in timeframe_shortlist:
        ...
        _timeframe_bases = timeframe_base_pattern(
            ohlcva,
            a_pattern_ohlcva,
            a_trigger_ohlcva,
            timeframe,
    ...
    _multi_timeframe_base_patterns = verify_backtesting_ability_by_base_timeframe_atr(
        _multi_timeframe_base_patterns, expanded_multi_timeframe_ohlcva
    )
...
def get_multi_timeframe_base_patterns(
    ...
    expanded_start = to_timeframe(
        start, anti_trigger_timeframe(timeframe_shortlist[-1]), ignore_cached_times=True, do_not_warn=True
    )
    expanded_date_range_str = date_range_to_string(end=end, start=expanded_start)
    multi_timeframe_ohlcva = get_multi_timeframe_ohlcva(expanded_date_range_str)
    base_patterns = multi_timeframe_base_patterns(multi_timeframe_ohlcva, timeframe_shortlist=timeframe_shortlist)
    base_patterns = trim_to_date_range(date_range_str, base_patterns)
    base_patterns = base_patterns.sort_index(level="date")
    return base_patterns


def first_passed_candle(
    base_index: datetime,
    end,
    level: float,
    base_timeframe_ohlcva: pt.DataFrame[OHLCVA],
    ...
    start = (
        base_index + pd.to_timedelta(timeframe) * app_config.base_pattern_index_shift_after_last_candle_in_the_sequence
    )
    passing_candles = (
        base_timeframe_ohlcva.sort_index(level="date")
        .loc[start:end]
        .loc[compare(level, base_timeframe_ohlcva[high_low])]
    )
...
def update_band_status(
    inactive_bases: pt.DataFrame[BasePattern],
    base_timeframe_ohlcva: pt.DataFrame[OHLCVA],
    timeframe,
    direction: Literal["upper", "below"],
) -> pt.DataFrame[BasePattern]:
    ...
    for start, base in inactive_bases.iterrows():
        inactive_bases.loc[start, f"{direction}_band_activated"] = first_passed_candle(
            start,
            base["ttl"],
            expand(base[f"internal_{high_low}"], base["atr"]),
            base_timeframe_ohlcva,
            timeframe,
            direction,
        )

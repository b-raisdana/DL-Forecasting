...
from archive_not_used_trash.domain.schemas.market_structure.BullBearSide import BullBearSide, MultiTimeframeBullBearSide, bull_bear_side_repr
from archive_not_used_trash.domain.schemas.market_structure.CandleTrend import CandleTrend, MultiTimeframeCandleTrend
from archive_not_used_trash.domain.schemas.market_structure.PeakValley import MultiTimeframePeakValley, PeakValley
from helper.data_preparation import concat, single_timeframe, to_timeframe
from archive_not_used_trash.helper.data_preparation import date_range_of_data
...
def insert_previous_n_next_tops(
    single_timeframe_peaks_n_valleys: pt.DataFrame[PeakValley], ohlcv: pt.DataFrame[OHLCV]
) -> pt.DataFrame[OHLCV]:
...
@profile_it
def single_timeframe_candles_trend(
    ohlcv: pt.DataFrame[OHLCV], timeframe_peaks_n_valley: pt.DataFrame[PeakValley]
    ...
    if len(candles_with_known_trend) == 0:
        ...
        if candle_trend["is_final"].isna().any():
            pass
        return candle_trend
    candle_trend.loc[candles_with_known_trend, "bull_bear_side"] = TREND.SIDE.value
...
@profile_it
def expand_trend_by_near_tops(
    timeframe_bull_or_bear: pt.DataFrame[BullBearSide],
    timeframe_peaks: pt.DataFrame[PeakValley],
    timeframe_valleys: pt.DataFrame[PeakValley],
    trend: TREND,
):
    ...
    while (
        ...
        timeframe_bull_or_bear = pd.merge_asof(
            timeframe_bull_or_bear.sort_values(by="movement_end_time"),
            shifted_next_tops,
            direction="forward",
            left_on="movement_end_time",
            right_index=True,
        )
        end_expandable_indexes = timeframe_bull_or_bear.loc[
            timeframe_bull_or_bear.index.isin(possible_end_expandable_indexes)
            & timeframe_bull_or_bear["next_top_value"].notna()
            & more_significant_end(
                timeframe_bull_or_bear["next_top_value"], timeframe_bull_or_bear[f"internal_{end_significant_column}"]
            )
        ].index
        ...
        timeframe_bull_or_bear.loc[end_expandable_indexes, "movement_end_time"] = timeframe_bull_or_bear.loc[
            end_expandable_indexes, "next_top_time"
        ]
        timeframe_bull_or_bear.loc[end_expandable_indexes, "movement_end_value"] = timeframe_bull_or_bear.loc[
            end_expandable_indexes, "next_top_value"
        ]
        ...
        timeframe_bull_or_bear = pd.merge_asof(
            timeframe_bull_or_bear.sort_values(by="movement_start_time"),
            shifted_previous_tops,
            direction="backward",
            left_on="movement_start_time",
            right_index=True,
        )
        start_expandable_indexes = timeframe_bull_or_bear.loc[
            timeframe_bull_or_bear.index.isin(possible_start_expandable_indexes)
            & timeframe_bull_or_bear["previous_top_value"].notna()
            & more_significant_start(
                timeframe_bull_or_bear["previous_top_value"],
        ...
        timeframe_bull_or_bear.loc[start_expandable_indexes, "movement_start_time"] = timeframe_bull_or_bear.loc[
            start_expandable_indexes, "previous_top_time"
        ]
        timeframe_bull_or_bear.loc[start_expandable_indexes, "movement_start_value"] = timeframe_bull_or_bear.loc[
            start_expandable_indexes, "previous_top_value"
        ]
        ...
        changed_ends = (
            ...
            .sort_index()
            .compare(previous_round_movement_end_time.sort_index())
        )
...
def shifted_time_and_value(
    df: pd.DataFrame, direction: Literal["next", "previous"], source_value_column: str, shifted_columns_prefix: str
) -> pd.DataFrame:
    if direction == "next":
        shift_value = -1
...
def add_trends_movement(
    bbs_trends: pt.DataFrame[BullBearSide],
...
def add_trend_movement(
    bull_or_bear_trends: pt.DataFrame[BullBearSide], timeframe_peaks_n_valleys: pt.DataFrame[PeakValley], trend: TREND
) -> pt.DataFrame[BullBearSide]:
    assert (
    ...
    if trend == TREND.BULLISH:
        ...
        raise Exception(f"Invalid trend['bull_bear_side']={trend}")
    next_start_tops = shifted_time_and_value(
        start_tops, "next", start_significant_column, f"{start_top_type}_after_start"
    )
    previous_end_tops = shifted_time_and_value(
        end_tops, "previous", end_significant_column, f"{end_top_type}_before_end"
    )
    bull_or_bear_trends = pd.merge_asof(
        bull_or_bear_trends, next_start_tops, left_index=True, right_index=True, direction="forward"
    )
    bull_or_bear_trends = pd.merge_asof(
        bull_or_bear_trends, previous_end_tops, left_on="end", right_index=True, direction="backward"
    )

    # invalid trends are trends without peak and valleys
    with_next_top_trends = bull_or_bear_trends[
        bull_or_bear_trends[f"next_{start_top_type}_after_start_time"] < bull_or_bear_trends["end"]
    ].index
    no_next_top_trends = bull_or_bear_trends.index.difference(with_next_top_trends)
    with_previous_top_trends = bull_or_bear_trends[
        bull_or_bear_trends[f"previous_{end_top_type}_before_end_time"] > bull_or_bear_trends.index
    ].index
...
@profile_it
def expand_trends_by_near_tops(
    _timeframe_bull_bear_side_trends: pt.DataFrame[BullBearSide],
    ...
    if len(_timeframe_bull_bear_side_trends) > 0:
        if not _timeframe_bull_bear_side_trends.index.is_unique:
            raise Exception(
                "We expect the single_timeframe_boundaries index be unique but isn't. "
        ...
        if len(bullish_indexes) > 0:
            _timeframe_bull_bear_side_trends.loc[bullish_indexes] = expand_trend_by_near_tops(
        ...
        if len(bearish_indexes) > 0:
            _timeframe_bull_bear_side_trends.loc[bearish_indexes] = expand_trend_by_near_tops(
...
@profile_it
def multi_timeframe_bull_bear_side_trends(
    multi_timeframe_candle_trend: pt.DataFrame[MultiTimeframeCandleTrend],
    ...
    for timeframe in timeframe_shortlist:
        ...
        timeframe_trends = single_timeframe_bull_bear_side_trends(
            timeframe_candle_trend,
...
def add_trend_extremum(
    _boundaries, single_timeframe_peak_n_valley: pt.DataFrame[PeakValley], ohlcv: pt.DataFrame[OHLCV]
):
    ...
    grouped_ohlcv = ohlcv.groupby(by="bbs_index").agg(
        {
            "low": "min",
            "high": "max",
        }
...
def trend_duration(boundaries: pt.DataFrame[BullBearSide]) -> pt.Series[timedelta]:
    ...
    if len(invali_length_boundaries) > 0:
        message_body = "\n".join(
            [
                BullBearSide.repr(start, boundary)
...
def trend_movement(boundaries: pt.DataFrame[BullBearSide]) -> pt.Series[float]:
    ...
    if len(boundaries) > 0:
        ...
        t = boundaries[["internal_high", "movement_start_value", "movement_end_value"]].max(
            axis="columns"
...
def previous_trend(trends: pt.DataFrame[BullBearSide]) -> tuple[list[int | None], list[float | None]]:
    """
...
def single_timeframe_bull_bear_side_trends(
    single_timeframe_candle_trend: pd.DataFrame, timeframe_peaks_n_valleys, ohlcva: pt.DataFrame[OHLCVA], timeframe: str
) -> pd.DataFrame:
...
def detect_trends(single_timeframe_candle_trend, timeframe: str) -> pt.DataFrame[BullBearSide]:
    ...
    if len(boundaries) > 1:
        boundaries.loc[: boundaries.index[-2], "end"] = pd.to_datetime(
            to_timeframe(boundaries.index.get_level_values("date")[1:], timeframe).tolist()
        )
...
def get_multi_timeframe_bull_bear_side_trends(
    ...
    multi_timeframe_candle_trend = get_multi_timeframe_candle_trend(date_range_str)
    trends = multi_timeframe_bull_bear_side_trends(
        multi_timeframe_candle_trend,
...
def get_multi_timeframe_candle_trend(
    ...
    multi_timeframe_ohlcv = get_multi_timeframe_ohlcv(date_range_str)
    multi_timeframe_peaks_n_valleys = get_multi_timeframe_peaks_n_valleys(date_range_str).sort_index(level="date")
    ...
    for timeframe in timeframe_shortlist:  # peaks_n_valleys.index.unique(level='timeframe'):
        t_candle_trend = single_timeframe_candles_trend(
            single_timeframe(multi_timeframe_ohlcv, timeframe),

...
def insert_passing_time(pivots: pt.DataFrame[Pivot2DFM], ohlcv: pt.DataFrame[OHLCV]):
    ...
    if app_config.check_assertions and any(
        pivots.loc[support_pivots.index, "external_margin"] > pivots.loc[support_pivots.index, "internal_margin"]
    ):
        raise AssertionError(
            "any(pivots.loc[support_pivots.index, 'external_margin'] > pivots.loc[support_pivots.index, "
            "'internal_margin'])"
        )
    pivots.loc[support_pivots.index, "passing_time"] = insert_crossing2(
        base=support_pivots,
        base_target_column="external_margin",
        target=ohlcv,
    ...
    if app_config.check_assertions and any(
        pivots.loc[resistance_pivots.index, "external_margin"] < pivots.loc[resistance_pivots.index, "internal_margin"]
    ):
        raise AssertionError(
            "any(pivots.loc[resistance_pivots.index, 'external_margin'] < "
            "pivots.loc[resistance_pivots.index, 'internal_margin'])"
        )
    pivots.loc[resistance_pivots.index, "passing_time"] = insert_crossing2(
        base=resistance_pivots,
        base_target_column="external_margin",
        target=ohlcv,
...
def deactivate_by_ttl(pivots: pt.DataFrame[Pivot2DFM], end: datetime):
    ...
    ttl_expired = pivots[pivots["deactivated_at"].isna() & (pivots["ttl"] < end)].index

    pivots.loc[ttl_expired, "deactivated_at"] = pivots.loc[ttl_expired, "ttl"]


def deactivate_on_passing_times(
    pivots: pt.DataFrame["Pivot2DFM"],
    ohlcv: pt.DataFrame[OHLCV],
):
    if "passing_time" not in pivots.columns:
        pivots["passing_time"] = pd.Series(dtype="datetime64[ns, UTC]")
    may_have_passing = pivots[pivots["deactivated_at"].isna()]
    may_have_passing.loc[:, ["passing_time", "deactivated_at"]] = insert_passing_time(may_have_passing, ohlcv)[
        ["passing_time", "deactivated_at"]
    ]
...
# @measure_time
def duplicate_on_passing_times(
    pivots: pt.DataFrame["Pivot2DFM"],
    ohlcv: pt.DataFrame[OHLCV],
):
    # todo: it is too slow!
    ...
    while len(may_have_passing) > 0:  # len(old_pivots_with_passing) < pivots[pivots['passing_time'].notna()]:
        ...
        passed_pivots = may_have_passing[may_have_passing["passing_time"].notna()]
        pivots.loc[passed_pivots.index, ["passing_time", "deactivated_at"]] = passed_pivots[
            ["passing_time", "deactivated_at"]
        ]
...
def inactivate_3rd_hit(pivots: pt.DataFrame["Pivot2DFM"], ohlcv: pt.DataFrame[OHLCV]):
    ...
    pivots.loc[deactivate_by_hit, "deactivated_at"] = pivots.loc[
        deactivate_by_hit, f"hit_end_{app_config.pivot_number_of_active_hits}"
    ]
...
def insert_is_overlap_of(multi_timeframe_pivots: pt.DataFrame[MultiTimeframePivotDFM]):
    ...
    for mt_index, pivot_info in multi_timeframe_pivots.iterrows():
        ...
        overlapped_pivots = multi_timeframe_pivots[(pivot_info["level"] < highs) & (pivot_info["level"] > lows)]
...
def update_hit(timeframe_pivots: pt.DataFrame[Pivot2DFM], ohlcv) -> pt.DataFrame[Pivot2DFM]:
    ...
    if app_config.check_assertions and "passing_time" not in timeframe_pivots.columns:
        AssertionError(
            "Expected passing_time be calculated before: 'passing_time' not in active_timeframe_pivots.columns"
        )
    ...
    timeframe_pivots["boundary_of_hit_end_0"] = nearest_match(
        needles=timeframe_pivots.loc[may_have_hit, ["return_end_time", "start_date"]].max(axis="columns").tolist(),
        reference=ohlcv.index,
        direction="right",
        shift=1,
    )
    timeframe_pivots.drop(columns="start_date", inplace=True)
    timeframe_pivots["hit_end_0"] = (insert_hits(timeframe_pivots, "boundary_of_hit_end_0", 0, ohlcv, "end"))[
        "hit_end_0"
    ]
    ...
    for n in range(1, app_config.pivot_number_of_active_hits + 1):
        ...
        # finding hit n start
        timeframe_pivots.loc[may_have_hit, f"boundary_of_hit_start_{n}"] = nearest_match(
            needles=timeframe_pivots.loc[may_have_hit, f"hit_end_{n - 1}"],
            reference=ohlcv.index,
            direction="right",
            shift=1,
        )
        timeframe_pivots.loc[may_have_hit, f"hit_start_{n}"] = (
            insert_hits(timeframe_pivots.loc[may_have_hit], f"boundary_of_hit_start_{n}", n, ohlcv, "start")
        )[f"hit_start_{n}"]
        invalid_hit_starts = timeframe_pivots[
            timeframe_pivots[f"hit_start_{n}"] > timeframe_pivots["passing_time"]
        ].index
        ...
        # finding hit n end
        timeframe_pivots.loc[started_pivots, f"boundary_of_hit_end_{n}"] = nearest_match(
            needles=timeframe_pivots.loc[started_pivots, f"hit_start_{n}"],
            reference=ohlcv.index,
            direction="right",
            shift=1,
        )
        timeframe_pivots.loc[started_pivots, f"hit_end_{n}"] = (
            insert_hits(timeframe_pivots.loc[started_pivots], f"boundary_of_hit_end_{n}", n, ohlcv, "end")
        )[f"hit_end_{n}"]
        invalid_hit_ends = timeframe_pivots[timeframe_pivots[f"hit_end_{n}"] > timeframe_pivots["passing_time"]].index
...
# @measure_time
def insert_hits(
    active_timeframe_pivots: pt.DataFrame[Pivot2DFM],
    hit_boundary_column: str,
    n: int,
    ohlcv: pt.DataFrame[OHLCV],
    ...
    resistance_ilocs = t_df.loc[
        t_df["is_resistance"].astype(bool) & t_df.index.get_level_values("date").notna(), "iloc"
    ].tolist()
    if t_df[t_df["iloc"].isin(resistance_ilocs)].index.get_level_values("date").isna().any():
        pass  # todo: test
    t_df.loc[t_df["iloc"].isin(resistance_ilocs), f"hit_{side}_{n}"] = insert_hit(
        t_df[t_df["iloc"].isin(resistance_ilocs)], n, ohlcv, side, "Resistance"
    )[f"hit_{side}_{n}"]
    ...
    if t_df[t_df["iloc"].isin(support_start_ilocs)].index.get_level_values("date").isna().any():
        pass  # todo: test
    t_df.loc[t_df["iloc"].isin(support_start_ilocs), f"hit_{side}_{n}"] = insert_hit(
        t_df.loc[t_df["iloc"].isin(support_start_ilocs)], n, ohlcv, side, "Support"
    )[f"hit_{side}_{n}"]
...
def insert_hit(
    ...
    ohlcv: pt.DataFrame[OHLCV],
    side: Literal["start", "end"],
    pivot_type: Literal["Resistance", "Support"],
):
    ...
    pivots = pivots.copy()
    if side == "start":
        pivots.loc[:, f"hit_start_{n}"] = insert_crossing2(
            base=pivots,
            base_target_column="internal_margin",
            target=ohlcv,
        ...
        pivots.loc[:, f"hit_end_{n}"] = insert_crossing2(
            base=pivots,
            base_target_column="internal_margin",
            target=ohlcv,
            target_compare_column=high_low,
...
# @measure_time
def insert_pivot_info(
    timeframe_pivots: pt.DataFrame["Pivot2DFM"],
    ohlcva: pt.DataFrame[OHLCVA],
    timeframe: str,
    pattern_ohlcva: pt.DataFrame[OHLCVA],
    trigger_ohlcva: pt.DataFrame[OHLCVA],
):
    timeframe_pivots["original_start"] = timeframe_pivots.index
    timeframe_pivots.set_index("original_start", append=True, inplace=True)

    timeframe_resistance_pivots = timeframe_pivots[timeframe_pivots["peak_or_valley"] == TopTYPE.PEAK.value].index
    timeframe_pivots.loc[timeframe_resistance_pivots, ["internal_margin", "external_margin"]] = pivot_margins(
        ...
        breakout_margin_atr=ohlcva,
    )[["internal_margin", "external_margin"]]

    timeframe_support_pivots = timeframe_pivots[timeframe_pivots["peak_or_valley"] == TopTYPE.VALLEY.value].index
    timeframe_pivots.loc[
        timeframe_support_pivots,
        [
            "internal_margin",
            "external_margin",
        ],
    ...
    timeframe_pivots = pd.merge_asof(
        left=timeframe_pivots, right=pattern_ohlcva, left_index=True, right_index=True, direction="backward"
    )
    trigger_ohlcva = trigger_ohlcva[["atr"]].rename(columns={"atr": "trigger_atr"})
    timeframe_pivots = pd.merge_asof(
        left=timeframe_pivots, right=trigger_ohlcva, left_index=True, right_index=True, direction="backward"
    )

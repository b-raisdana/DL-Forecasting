...
import pandas as pd
from archive_not_used_trash.domain.price_action.ftc import insert_multi_timeframe_pivots_real_start
from archive_not_used_trash.domain.schemas.market_structure.AtrTopPivot import (
    AtrMovementPivotDf,
    MultiTimeframeAtrMovementPivotDf,
...
@profile_it
def find_multi_timeframe_atr_movement_pivots(
    mt_ohlcva, base_timeframe_ohlcva, mt_tops, structure_timeframe_shortlist, same_time_multiple_timeframes
):
    ...
    for timeframe in structure_timeframe_shortlist[::-1]:
        if timeframe in mt_tops.index.get_level_values("timeframe"):
            ...
            timeframe_tops = pd.merge_asof(
                timeframe_tops, ohlcva[["atr"]], left_index=True, right_index=True, direction="backward"
            )
            insert_pivot_movements(timeframe_tops, base_timeframe_ohlcva)

            movement_satisfied_tops = timeframe_tops[
                timeframe_tops["movement_start_time"].notna() & timeframe_tops["return_end_time"].notna()
            ].copy()
            if len(movement_satisfied_tops) > 0:
                ...
                timeframe_pivots = movement_satisfied_tops[
                    (
                        ...
                        | (
                            movement_satisfied_tops["left_more_significant_top"]
                            <= movement_satisfied_tops["movement_start_time"]
                        )
                    )
                    & (
                        movement_satisfied_tops["right_more_significant_top"].isna()
                        | (
                            movement_satisfied_tops["right_more_significant_top"]
                            >= movement_satisfied_tops["return_end_time"]
                        )
                    )
                ].copy()
...
def generate_multi_timeframe_atr_movement_pivots(
    ...
    t_atr_movement_pivots = atr_movement_pivots(
        date_range_str=date_range_str, structure_timeframe_shortlist=timeframe_shortlist
    )
    return t_atr_movement_pivots


def read_multi_timeframe_atr_movement_pivots(
    date_range_str: str = None,
) -> pt.DataFrame[MultiTimeframeAtrMovementPivotDFM]:
    if date_range_str is None:
        date_range_str = app_config.processing_date_range
    result = MultiTimeframeAtrMovementPivotDf.read_file(
        date_range_str, "multi_timeframe_atr_movement_pivots", generate_multi_timeframe_atr_movement_pivots
    )
    return result


@profile_it
def atr_movement_pivots(
    date_range_str: str = None,
    structure_timeframe_shortlist: list["str"] = None,
    same_time_multiple_timeframes: bool = True,
) -> pt.DataFrame[MultiTimeframeAtrMovementPivotDFM]:
    """
    for peaks:
        (
    ...
    mt_tops_mapped_time = [
        to_timeframe(date, timeframe, ignore_cached_times=True, do_not_warn=True)
    ...
    pivots = find_multi_timeframe_atr_movement_pivots(
        mt_ohlcva, base_timeframe_ohlcva, mt_tops, structure_timeframe_shortlist, same_time_multiple_timeframes
    )
    ...
    for timeframe in structure_timeframe_shortlist[::-1][:-1]:
        ...
        if app_config.check_assertions and len(timeframe_pivots) != len(same_time_pattern_timeframe_pivots):
            raise AssertionError(
                "Expected every pivot overlaps with a pivot in pattern time. "
...
@profile_it
def insert_pivot_movements(
    timeframe_tops: pt.DataFrame[PeakValley], base_ohlcv: pt.DataFrame[OHLCV]
    ...
    timeframe_tops.loc[
        peaks_indexes, ["right_crossing_time", "left_crossing_time", "right_crossing_value", "left_crossing_value"]
    ] = insert_pivot_movement_times(peaks, base_ohlcv, top_type=TopTYPE.PEAK)[
        ["right_crossing_time", "left_crossing_time", "right_crossing_value", "left_crossing_value"]
    ]

    valleys = valleys_only(timeframe_tops).copy()
    valleys_indexes = valleys.index
    timeframe_tops.loc[
        valleys_indexes, ["right_crossing_time", "left_crossing_time", "right_crossing_value", "left_crossing_value"]
...
@profile_it
def insert_pivot_movement_times(
    timeframe_peak_or_valleys: pt.DataFrame[PeakValley], base_ohlcv: pt.DataFrame[OHLCV], top_type: TopTYPE
    ...
    peak_or_valley_indexes = timeframe_peak_or_valleys[
        timeframe_peak_or_valleys["peak_or_valley"] == top_type.value
    ].index
    peak_or_valleys = timeframe_peak_or_valleys.loc[peak_or_valley_indexes]

    peak_or_valleys["previous_target"] = aggregate(
        peak_or_valleys[high_low], 3 * timeframe_peak_or_valleys.loc[peak_or_valley_indexes, "atr"]
    )
    if top_type == TopTYPE.PEAK:
        timeframe_peak_or_valleys.loc[peak_or_valley_indexes, ["left_crossing_time", "left_crossing_value"]] = (
            insert_crossing2(
                base=peak_or_valleys,
                base_target_column="previous_target",
                direction="left",
                target=base_ohlcv,
    ...
    if top_type == TopTYPE.PEAK:
        timeframe_peak_or_valleys.loc[peak_or_valley_indexes, ["right_crossing_time", "right_crossing_value"]] = (
            insert_crossing2(
                base=peak_or_valleys,
                base_target_column="next_target",
                direction="right",
                target=base_ohlcv,
                target_compare_column="low",
                more_significant=lambda target, base: target < base,
...
@profile_it
def insert_more_significant_tops(
    timeframe_tops: pt.DataFrame[PeakValley], compared_tops: pt.DataFrame[MultiTimeframePeakValley]
) -> pt.DataFrame[MultiTimeframePeakValley]:
    timeframe_tops.drop(
        columns=["right_crossing_time", "left_crossing_time", "right_crossing_value", "left_crossing_value"],
        inplace=True,
...
def insert_more_significant_top(
    base_tops: pt.DataFrame[MultiTimeframePeakValley],
    ...
    if top_type == TopTYPE.PEAK:
        base_tops.loc[peak_or_valley_indexes, ["left_crossing_time", "left_crossing_value"]] = insert_crossing2(
            base=base_tops.loc[peak_or_valley_indexes],
            target=no_timeframe_tops,
            direction="left",
            target_compare_column="high",
            base_target_column="target",
            more_significant=lambda target, base: target > base,
        )[["left_crossing_time", "left_crossing_value"]]
        base_tops.loc[peak_or_valley_indexes, ["right_crossing_time", "right_crossing_value"]] = insert_crossing2(
            base=base_tops.loc[peak_or_valley_indexes],
            target=no_timeframe_tops,
            direction="right",
            target_compare_column="high",

...
# @measure_time
def pivots_level_n_margins(
    timeframe_pivots: pd.DataFrame,
    pivot_time_peaks_n_valleys: pd.DataFrame,
    timeframe: str,
    ...
    """
    if hasattr(pivot_time_peaks_n_valleys.index, "names") and "timeframe" in pivot_time_peaks_n_valleys.index.names:
        raise ValueError(
            "Expected single-timeframe pivot_time_peaks_n_valleys "
    ...
    if hasattr(timeframe_pivots.index, "names") and "timeframe" in timeframe_pivots.index.names:
        raise ValueError("Expected single-timeframe timeframe_pivots but 'timeframe' in timeframe_pivots.index.names")
    pivot_time_peaks = peaks_only(pivot_time_peaks_n_valleys)
    timeframe_pivots = peak_or_valley_pivots_level_n_margins(
        timeframe_pivots,
        pivot_time_peaks,
        TopTYPE.PEAK,
        timeframe,
...
def peak_or_valley_pivots_level_n_margins(
    timeframe_pivots: pd.DataFrame,
    pivot_time_peaks_or_valleys: pd.DataFrame,
    _type: TopTYPE,
    timeframe: str,
    candle_body_source: pd.DataFrame,
    ...
    """
    if hasattr(pivot_time_peaks_or_valleys.index, "names") and "timeframe" in pivot_time_peaks_or_valleys.index.names:
        raise ValueError(
            "Expected single-timeframe no_timeframe_pivot_time_peaks_or_valleys "
    ...
    timeframe_pivots = timeframe_pivots.merge(
        pivot_time_peaks_or_valleys[["level_y"]], how="left", left_index=True, right_index=True
    )
    timeframe_pivots.loc[pivot_time_peaks_or_valleys.index, "level"] = timeframe_pivots.loc[
        pivot_time_peaks_or_valleys.index, "level_y"
    ]
    ...
    timeframe_pivots = pivot_margins(
        timeframe_pivots, _type, pivot_time_peaks_or_valleys, candle_body_source, breakout_margin_atr
    )
    return timeframe_pivots
...
def pivot_margins(
    pivots: pd.DataFrame,
    _type: TopTYPE,  # pivot_peaks_or_valleys: pd.DataFrame,
    candle_body_source: pd.DataFrame,
    breakout_margin_atr: pd.DataFrame,
) -> pd.DataFrame:
    """
    ...
    candle_body_source["nearest_body"] = candle_body_source[["open", "close"]].apply(
        choose_body_operator, axis="columns"
    )

    pivots.reset_index(level="original_start", inplace=True)
    if pivots.index.duplicated().any():
        raise ValueError(
            "pivots.index.duplicated().any() maybe running after duplicating, should be run before duplicating!"
        )
    pivots = pd.merge_asof(
        pivots, candle_body_source, left_index=True, right_index=True, direction="backward", suffixes=("_x", "")
    )
    ...
    if _type.value == TopTYPE.PEAK.value:
        ...
        if app_config.check_assertions and any(pivots["external_margin"] > pivots["internal_margin"]):
            raise AssertionError(
                "config.check_assertions and any(pivots.loc[pivot_times, 'external_margin'] > "

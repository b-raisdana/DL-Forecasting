...
def load_peak_n_valleys() -> pd:
    peaks = pd.read_csv(
        "archive/peaks.17-10-05.18-40T17-10-06.11-19.zip", index_col="date", header=0, parse_dates="date"
    )
    peaks["peak_or_valley"] = TopTYPE.PEAK.value
    valleys = pd.read_csv(
        "archive/valleys.17-10-05.18-40T17-10-06.11-19.zip", index_col="date", header=0, parse_dates="date"
    )
...
def rally_base_drop(peaks_n_valleys: pd):
    ...
    trend["RDB"] = (
        RALLY_TREND
        if (trend.next_valley > trend.previous_valley and trend.previous_peak > trend.next_peak)
        else DROP_TREND
        if (trend.next_valley < trend.previous_valley and trend.previous_peak < trend.next_peak)
        else BASE_TREND
    )

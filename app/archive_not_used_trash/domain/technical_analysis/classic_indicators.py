...
def add_classic_indicators(ohlcv):
    ...
    ohlcv["mfi"] = ta.mfi(high=ohlcv["high"], low=ohlcv["low"], close=ohlcv["close"], volume=ohlcv["volume"]).astype(
        float
    )
...
# duplicated from app/domain/technical_analysis/classic_indicators.py (still live there; a dead function here depends on it)

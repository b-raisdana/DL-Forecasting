...
def load_and_predict(input_x):
    ...
    predictions = model.predict(
        {
            "structure_model_input": input_x["structure"],
...
def prediction_reconstructor(row, prev_row):
    ...
    return pd.Series(
        [high, low, mean_high, mean_low, std_high, std_low],
        index=["high", "low", "mean_high", "mean_low", "std_high", "std_low"],
    )


def reconstruct_from_prediction(
    last_x_stats, last_x_double_datetime, predictions: np.array, predictions_tf, forecast_horizon: int = 20
):
    ...
    reconstructed = pd.DataFrame(
        predictions,
        columns=training_y_columns,
        index=pd.date_range(
            last_x_double_datetime + pd.to_timedelta(predictions_tf), periods=predictions.shape[0], freq=predictions_tf
        ),
    )
    ...
    reconstructed.iloc[0, reconstructed.columns.get_loc("mean_high")] = last_x_stats["mean_high"]
    ...
    for i in range(1, forecast_horizon):
        ...
        reconstructed.loc[reconstructed.index[i], ["high", "low", "mean_high", "mean_low", "std_high", "std_low"]] = (
            prediction_reconstructor(current_row, prev_row)
        )[["high", "low", "mean_high", "mean_low", "std_high", "std_low"]]

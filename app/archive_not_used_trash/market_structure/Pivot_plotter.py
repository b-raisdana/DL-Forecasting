...
# @measure_time
def plot_multi_timeframe_pivots(
    mt_pivots: pt.DataFrame[MultiTimeframePivot2DFM],
    ...
    resistance_timeframes = mt_pivots[mt_pivots["is_resistance"]].index.get_level_values(level="timeframe").unique()
    support_timeframes = mt_pivots[~mt_pivots["is_resistance"]].index.get_level_values(level="timeframe").unique()
    ...
    if not show_duplicates:
        mt_pivots = mt_pivots[
            mt_pivots.index.get_level_values("date") == mt_pivots.index.get_level_values("original_start")
        ]
    ...
    for (pivot_timeframe, pivot_start, pivot_original_start), pivot_info in mt_pivots.iterrows():
        ...
        if pivot_name not in plotted_pivots:
            ...
            if pd.isnull(pivot_info["ttl"]):
                raise Exception(
                    f"Every level should have a ttl.{pivot_timeframe}, {pivot_start}, {pivot_info} ttl is Null"
                )
            ...
            if show_boundaries:
                ...
                fig.add_scatter(
                    ...
                    y=[
                        pivot_info["external_margin"],
                        pivot_info["external_margin"],
                        pivot_info["internal_margin"],
                        pivot_info["internal_margin"],
                    ],
                    fill="toself",
                    opacity=0.2,
                    ...
                    legendgroup=legend_group,
                    showlegend=False,
                    hoverinfo="text",  # text=pivot_description,
                )
            if pivot_start == pivot_original_start:
                # add movement and return paths
                if (
                    hasattr(pivot_info, "movement_start_time")
                    ...
                    fig.add_scatter(
                        x=[
                            pivot_info["movement_start_time"],
                            pivot_start,
                        ],
                        y=[
                            pivot_info["movement_start_value"],
                            pivot_info["level"],
                        ],
                        name=pivot_name,
                ...
                if hasattr(pivot_info, "real_start_time") and hasattr(pivot_info, "real_start_value"):
                    fig.add_scatter(
                        x=[
                            pivot_info["real_start_time"],
                            pivot_start,
                        ],
                        y=[
                            pivot_info["real_start_value"],
                            pivot_info["level"],
                        ],
                        name=pivot_name,

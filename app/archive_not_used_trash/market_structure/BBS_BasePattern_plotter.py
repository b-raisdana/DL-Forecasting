...
@profile_it
def plot_multi_timeframe_bbs_n_base_pattern(
    multi_timeframe_base_pattern: pt.DataFrame[MultiTimeframeBasePattern],
    ...
    multi_timeframe_base_pattern["effective_end"] = multi_timeframe_base_pattern[["end", "ttl"]].min(
        axis=1, skipna=True
    )
    ...
    for timeframe in timeframe_shortlist:
        ...
        lower_timeframe_base_pattern = multi_timeframe_base_pattern[
            multi_timeframe_base_pattern.index.get_level_values(level="timeframe").isin(
                app_config.timeframes[: app_config.timeframes.index(timeframe)]
            )
        ]
        for (base_timeframe, base_date), base_info in lower_timeframe_base_pattern.iterrows():
            ...
            real_start = (
                base_date
                + pd.to_timedelta(base_timeframe)
                * app_config.base_pattern_index_shift_after_last_candle_in_the_sequence
            )
            legend_group, text = draw_base(
                base_info=base_info, fig=fig, index_date=base_date, real_start=real_start, base_timeframe=base_timeframe
            )
            draw_band_activators(base_info, fig, legend_group, real_start, text)
            if (orders_df is not None) and (not orders_df.empty):
                draw_band_orders_df(
                    orders_df, fig, end_of_dates, end_of_prices, base_date, base_timeframe, legend_group
                )

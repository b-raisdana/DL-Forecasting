...
@profile_it
def plot_multi_timeframe_base_pattern(
    ...
    _multi_timeframe_base_pattern["effective_end"] = _multi_timeframe_base_pattern[["end", "ttl"]].min(
        axis=1, skipna=True
    )
    _multi_timeframe_base_pattern.loc[
        _multi_timeframe_base_pattern["effective_end"] > end_of_dates, "effective_end"
    ] = end_of_dates
    ...
    for (timeframe, index_date), base_pattern in _multi_timeframe_base_pattern.iterrows():
        if timeframe in app_config.timeframes:  # [2:-2]:
            ...
            if (orders_df is not None) and (not orders_df.empty):
                draw_band_orders_df(orders_df, fig, end_of_dates, end_of_prices, index_date, timeframe, legend_group)
...
def draw_band_orders_df(
    orders_df: pd.DataFrame,
    fig: plotly.graph_objs.Figure,
    end_of_dates: datetime,
    end_of_prices: float,
    base_pattern_date: pd.Timestamp,
    ...
    for order_group_id in order_group_ids:
        ...
        if (
            app_config.check_assertions
            and len(original_order) == 0
            or len(stop_loss_order) == 0
            or len(take_profit_order) == 0
        ):
        ...
        if started:
            ...
            end, end_price, ended = order_end_information(
                end_of_dates, end_of_prices, stop_loss_order, take_profit_order
            )
            ...
            draw_base_pattern_orders(
                ...
                stop_loss_price,
                take_profit_price,
                float(original_order["created_size"]),
...
def draw_base(base_info, fig, index_date, real_start, timeframe, legendgroup=None):
    ...
    ys = [base_info["internal_low"], base_info["internal_low"], base_info["internal_high"], base_info["internal_high"]]
    fill_color = "cyan"
    ...
    fig.add_scatter(
        x=[real_start, base_info["effective_end"]],
        y=[base_info["internal_low"], base_info["internal_low"]],
        mode="lines",
        line=dict(color=fill_color, width=0),  # fill="toself",
        showlegend=False,
        legendgroup=legendgroup,
        text=[name] * 2,
        hoverinfo="text",

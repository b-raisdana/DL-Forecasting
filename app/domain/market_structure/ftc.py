...
from domain.schemas.market_structure.Pivot import PivotDFM
...
def merge_bbs_overlap(time_frame_bbs: pt.DataFrame[BullBearSide]):
    ...
    next_movements = time_frame_bbs.copy()[["movement_start_time", "movement_end_time"]].rename(
        columns={"movement_start_time": "next_movement_start_time", "movement_end_time": "next_movement_end_time"}
    )
    next_movements["shifted_movement_start_time"] = next_movements["next_movement_start_time"] - pd.to_timedelta(
        app_config.timeframes[0]
    )
    ...
    merged_to_find_same_start = time_frame_bbs.merge(
        next_movements, left_on="movement_start_time", right_on="next_movement_start_time", how="left"
    )
    ...
    if len(valid_same_start_overlaps["next_movement_index"].dropna().unique()) > 0:
        pass
    time_frame_bbs.drop(labels=valid_same_start_overlaps["next_movement_index"].dropna().unique(), inplace=True)
    next_movements.drop(labels=valid_same_start_overlaps["next_movement_index"].dropna().unique(), inplace=True)
    # find overlaps which does not have the same start
    merged_to_find_overlap = pd.merge_asof(
        left=time_frame_bbs.sort_values(by="movement_start_time"),
        right=next_movements,
        left_on="movement_start_time",
        right_on="shifted_movement_start_time",
        direction="forward",
    )
    ...
    if len(no_merge_overlap_movements) > 0:
        time_frame_bbs.drop(labels=no_merge_overlap_movements["next_movement_index"].dropna().unique(), inplace=True)
    ...
    # ]
    to_merge_overlap_movements = overlapped_movements.loc[
        overlapped_movements.index.difference(no_merge_overlap_movements.index)
    ]
    if len(to_merge_overlap_movements) > 0:
        time_frame_bbs.loc[to_merge_overlap_movements["date_backup"], "movement_end_time"] = to_merge_overlap_movements[
            "next_movement_end_time"
        ].to_list()
        # drop trends which covered by the original after expansion.
        time_frame_bbs.drop(labels=to_merge_overlap_movements["next_movement_index"].dropna().unique(), inplace=True)
    return time_frame_bbs
    ...
    for timeframe in pivot_lower_timeframes:
        if len(remained_pivots) > 0:
            ...
            for pivot_start, pivot in remained_pivots.iterrows():
                ...
                pivots_with_ftc_range_start_time.loc[pivot_start, "ftc_list"] = remained_pivots.loc[
                    pivot_start, "ftc_list"
                ]
...
def ftc_of_range_by_price(
    pivots: pt.DataFrame[Pivot2DFM], mt_base_patterns: pt.DataFrame[MultiTimeframeBasePattern], pivots_timeframe
):
    ...
    pivots["ftc_range_start_value"] = pivots["level"] - pivots["real_movement"] * app_config.ftc_price_range_percentage
        ...
        if len(remained_pivots) > 0:
            ...
            for pivot_start, pivot_info in remained_pivots.iterrows():
                ...
                if len(matched_bases) > 0:
                    ...
                    pivots.at[pivot_start, "ftc_list"] = remained_pivots.loc[pivot_start, "ftc_list"]
            remained_pivots = remained_pivots[remained_pivots["ftc_list"].isna()]
        else:
            break


def insert_multi_timeframe_pivots_real_start(
...
def multi_timeframe_ftc(
    ...
    if timeframe_shortlist is None:
        ...
        timeframe_shortlist = [
            timeframe for timeframe in app_config.structure_timeframes[::-1] if timeframe in timeframe_shortlist
        ]
...
def insert_pivot_real_start(
    support_resistance: pt.DataFrame[PivotDFM], peaks_or_valleys: pt.DataFrame[PeakValley], top_type: TopTYPE
):
    """
    todo:
    ...
    for pivot_time, pivot in support_resistance.iterrows():
        passed_tops = peaks_or_valleys[
            peaks_or_valleys.index.get_level_values("date") <= pivot["movement_start_time"]
...
def zz_ftc_range_start_time(pivots_with_bbs_movement, ohlcv):
    ...
    pivots_with_bbs_movement.loc[resistance_pivots.index, "ftc_range_start_time"] = (
        insert_crossing2(
            ...
            base_target_column="ftc_range_start_value",
            target=ohlcv,

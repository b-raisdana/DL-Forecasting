...
def reverse_mt_rolling_mean_std(mt_rolling_mean_std):
    ...
    for timeframe in mt_rolling_mean_std.index.get_level_values(level="timeframe").unique():
        rolling_mean_std = mt_rolling_mean_std.loc[pd.IndexSlice[timeframe, :], :]
        for col in columns_list:
...
def get_multi_timeframe_rolling_mean_std_ohlcv(date_range_str: str = None) -> MtRollingMeanStdOHLCV:
    ...
    expanded_date_range = expand_date_range(date_range_str, time_delta=expander_duration, mode="start")
    mt_ohlcv = get_multi_timeframe_ohlcv(expanded_date_range)
